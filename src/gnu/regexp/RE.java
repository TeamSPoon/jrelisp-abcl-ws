...
public class RE extends REToken {
	...
	private static ResourceBundle messages = PropertyResourceBundle.getBundle("gnu/regexp/MessagesBundle",
			Locale.getDefault());
	...
	/**
	 * Compilation flag. Do not differentiate case. Subsequent searches using
	 * this RE will be case insensitive.
	 */
	public static final int REG_ICASE = 2;

	/**
	 * Compilation flag. The match-any-character operator (dot) will match a
	 * newline character. When set this overrides the syntax bit RE_DOT_NEWLINE
	 * (see RESyntax for details). This is equivalent to the "/s" operator in
	 * Perl.
	 */
	public static final int REG_DOT_NEWLINE = 4;

	/**
	 * Compilation flag. Use multiline mode. In this mode, the ^ and $ anchors
	 * will match based on newlines within the input. This is equivalent to the
	 * "/m" operator in Perl.
	 */
	 ...
	 * beginning of the input string. Useful for matching on a substring when
	 * you know the context of the input is such that position zero of the input
	 * to the match test is not actually position zero of the text.
	 * <P>
	...
	/**
	 * Execution flag. The match-end operator ($) does not match at the end of
	 * the input string. Useful for matching on substrings.
	 */
	public static final int REG_NOTEOL = 32;

	/**
	 * Execution flag. When a match method is invoked that starts matching at a
	 * non-zero index into the input, treat the input as if it begins at the
	 * index given. The effect of this flag is that the engine does not "see"
	 * any text in the input before the given index. This is useful so that the
	 * match-beginning operator (^) matches not at position 0 in the input
	 * string, but at the position the search started at (based on the index
	 * input given to the getMatch function). See the example under REG_NOTBOL.
	 * It also affects the use of the \&lt; and \b operators.
	 */
	public static final int REG_ANCHORINDEX = 64;

	/**
	 * Execution flag. The substitute and substituteAll methods will not attempt
	 * to interpolate occurrences of $1-$9 in the replacement text with the
	 ...
	 * @param cflags
	 *            The logical OR of any combination of the compilation flags
	 *            listed above.
	 * @exception REException
	 *                The input pattern could not be parsed.
	 * @exception NullPointerException
	...
	protected void initialize(Object patternObj, int cflags, RESyntax syntax, int myIndex, int nextSub)
		...
		while (index < pLength) {
			...
			// TODO: the '\n' literal here should be a test against
			// REToken.newline,
	...
	/**
	 * Returns an array of all matches found in the input, beginning at the
	...
	/**
	 * Returns an array of all matches found in the input string, beginning at
	 * the specified index position and using the specified execution flags.
	 *
	 * If the regular expression allows the empty string to match, it will
	 * substitute matches at all positions except the end of the input.
	 *
	...
	private REMatch[] getAllMatchesImpl(CharIndexed input, int index, int eflags) {
		...
		while ((m = getMatchImpl(input, index, eflags, null)) != null) {
			all.addElement(m);
			index = m.getEndIndex();
			if (m.end[0] == 0) { // handle pathological case of zero-length
									// match
				index++;
				input.move(1);
			} else {
	...
	/**
	 * Returns the first match found in the input. If no match is found, null is
	 * returned.
	 *
	 * @param input
	 *            The input text.
	...
	/**
	 * Returns the first match found in the input, beginning the search at the
	 * specified index. If no match is found, returns null.
	 *
	 * @param input
	 *            The input text.
	 * @param index
	 *            The offset within the text to begin looking for a match.
	 * @return An REMatch instance referencing the match, or null if none.
	 */
	...
	/**
	 * Returns the first match found in the input, beginning the search at the
	 * specified index, and using the specified execution flags. If no match is
	 * found, returns null.
	 *
	 * @param input
	 *            The input text.
	 ...
	 * Returns the first match found in the input, beginning the search at the
	 * specified index, and using the specified execution flags. If no match is
	 * found, returns null. If a StringBuffer is provided and is non-null, the
	 * contents of the input text from the index to the beginning of the match
	 * (or to the end of the input, if there is no match) are appended to the
	 * StringBuffer.
	 *
	 * @param input
	 *            The input text.
	 ...
	 *            The logical OR of any execution flags above.
	 * @param buffer
	 *            The StringBuffer to save pre-match text in.
	 * @return An REMatch instance referencing the match, or null if none.
	 */
		...
		do {
			...
			if (minimumLength == 0 || input.charAt(minimumLength - 1) != CharIndexed.OUT_OF_BOUNDS) {
				if (match(input, mymatch)) {
					// Find longest match of them all to observe leftmost
					// longest
	 ...
	 * @param index
	 *            The offset index at which the search should be begin.
	 * @return A non-null REMatchEnumeration instance, with its input cursor set
	 *         to the index position specified.
	 */
	 ...
	 *            The input text.
	 * @param replace
	 *            The replacement text, which may contain $x metacharacters (see
	 ...
	 * @param index
	 *            The offset index at which the search should be begin.
	 * @return A String containing the substring of the input, starting at the
	 *         index position, and interpolating the substituted text.
	 * @see REMatch#substituteInto
	 */
	...
	/**
	 * Substitutes the replacement text for the first match found in the input
	 * string, beginning at the specified index position and using the specified
	 * execution flags.
	 *
	 * @param input
	 *            The input text.
	...
	/**
	 * Substitutes the replacement text for each non-overlapping match found in
	 * the input text.
	 *
	 * @param input
	 *            The input text.
	...
	/**
	 * Substitutes the replacement text for each non-overlapping match found in
	 * the input text, starting at the specified index.
	 *
	 * If the regular expression allows the empty string to match, it will
	 * substitute matches at all positions except the end of the input.
	 *
	...
	/**
	 * Substitutes the replacement text for each non-overlapping match found in
	 * the input text, starting at the specified index and using the specified
	 * execution flags.
	 *
	 * @param input
	 *            The input text.
	...
	/**
	 * Return a human readable form of the compiled regular expression, useful
	 * for debugging.
	 */
