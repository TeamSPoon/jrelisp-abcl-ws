...
public class DirectoryTreeModel extends DefaultTreeModel {
	...
	public static DirectoryTreeModel getTreeModel(File file) {
		...
		DefaultMutableTreeNode root = new DefaultMutableTreeNode(new DirectoryTreeElement(rootFile));
		...
		if (Platform.isPlatformWindows()) {
			...
			if (roots != null) {
				...
				for (int i = 0; i < roots.length; i++) {
					DefaultMutableTreeNode child = new DefaultMutableTreeNode(new DirectoryTreeElement(roots[i]));
	...
	private static void addChildren(File parent, DefaultMutableTreeNode node) {
		...
		if (list == null)
			return;
		Arrays.sort(list, ignoreCase ? ciFileNameComparator : csFileNameComparator);
		for (int i = 0; i < list.length; i++) {
			File f = list[i];
			if (f.isDirectory() && !f.isLink()) {
				DefaultMutableTreeNode child = new DefaultMutableTreeNode(new DirectoryTreeElement(f));
	...
	private static DefaultMutableTreeNode findMatchingChild(DefaultMutableTreeNode parent, File file) {
		if (file != null) {
