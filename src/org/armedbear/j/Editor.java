...
public final class Editor extends JPanel implements Constants, ComponentListener, MouseWheelListener {
			...
			if (arg.startsWith("-")) {
				...
				if (arg.startsWith("--home")) {
					...
					if (userHomeDir == null || !userHomeDir.isDirectory()) {
						fatal("Specified home directory \"" + userHomeDir.canonicalPath() + "\" does not exist.");
					}

					if (!userHomeDir.canWrite()) {
						fatal("Specified home directory \"" + userHomeDir.canonicalPath() + "\" is not writable.");
					}
	...
	public void dropBookmark() {
		...
		if (e != null && e.getID() == KeyEvent.KEY_PRESSED) {
			...
			if (index >= 0 && index < bookmarks.length) {
				if (bookmarks[index] == null || confirm("Drop Bookmark", "Overwrite existing bookmark?")) {
	...
	public void backspace() {
		...
		try {
			if (mark != null) {
				...
				// The caret is beyond the end of the actual text on the current
				// line.
		...
		if (pos != null) {
			...
			if (match != null) {
				...
				if (pos.getLine() != match.getLine()) {
					...
					String trim = begin.getLine().substring(0, begin.getOffset()).trim();
					if (trim.length() == 0) {
	...
	public void closeParen() {
		...
		Position match = findMatchInternal(new Position(getDotLine(), getDotOffset() - 1), 50);
		if (match == null)
			return;
	...
	public void newlineAndIndent() {
		...
		try {
			...
			if (mode.canIndent()) {
				...
				// Can't indent according to context. Match indentation of
				// previous line.
	...
	private void saveAs(Buffer toBeSaved) {
		...
		if (toBeSaved.getType() == Buffer.TYPE_NORMAL) {
			...
			File destination = SaveFileDialog.getSaveFile(this, dialogTitle);
			if (destination == null)
				return;
	...
	public void saveCopy() {
		...
		if (buffer.getType() == Buffer.TYPE_NORMAL) {
			...
			final File destination = SaveFileDialog.getSaveFile(this, dialogTitle);
			if (destination == null)
				return;
	...
	public boolean execute(String commandName, String parameters) throws NoSuchMethodException {
		...
		try {
			...
			if (command != null) {
				...
				if (className == null) {
					// Special case. Command is implemented in
	...
	public Object[] getKeyMapping(String command) {
		...
		if (mapping == null) {
			...
			if (mapping != null) {
				javax.swing.KeyStroke keyStroke = javax.swing.KeyStroke.getKeyStroke(mapping.getKeyCode(),
	...
	private void selectToPosition(Position pos) {
		if (!pos.equals(dot) || buffer.getCol(pos) != display.getAbsoluteCaretCol()) {
			addUndo(SimpleEdit.MOVE);
			if (mark == null)
				setMarkAtDot();
	...
	public void home() {
		...
		if (mark != null || !pos.equals(dot) || buffer.getCol(pos) != display.getAbsoluteCaretCol()) {
			moveDotTo(pos);
	...
	public void insertChar(char c) {
		...
		try {
			addUndo(SimpleEdit.LINE_EDIT);
			fillToCaret();
			FastStringBuffer sb = new FastStringBuffer(dotLine.substring(0, getDotOffset()));
			sb.append(c);
	...
	public void insertByte() {
		...
		if (c >= 0 && c <= 255) {
			...
			try {
				...
				Log.error(e);
				MessageDialog.showMessageDialog(this, "Unsupported encoding \"" + encoding + "\"", "Insert Byte");
			}
		} else
			MessageDialog.showMessageDialog(this, "Invalid byte \"" + input + "\"", "Insert Byte");
	}
	...
	public void electricSemi() {
		if (!checkReadOnly())
			return;
		if (mark != null || getDotLine().flags() == STATE_COMMENT || getMode().isInQuote(buffer, dot)) {
			insertNormalChar(';');
		} else {
			CompoundEdit compoundEdit = beginCompoundEdit();
			insertChar(';');
		...
		if (mark == null) {
			...
			if (modeId == XML_MODE || modeId == HTML_MODE) {
				...
				if (buffer.getBooleanProperty(Property.AUTO_INDENT)) {
					if (modeId == HTML_MODE && getDotLine().substring(0, getDotOffset()).endsWith("</pre>")) {
		...
		if (lastSearch != null) {
			...
			if (pos != null) {
				...
				if (lastSearch instanceof FindInFiles) {
					if (buffer.getFile() != null) {
						ListOccurrencesInFiles buf = ((FindInFiles) lastSearch).getOutputBuffer();
						if (buf != null)
	...
	public void findFirstOccurrence() {
		...
		if (pattern == null || pattern.length() == 0)
			return;
		lastSearch = new Search(pattern, false, true);
		Position pos = lastSearch.find(buffer.getMode(), new Position(buffer.getFirstLine(), 0));
		if (pos != null) {
			moveDotTo(pos);
			markFoundPattern(lastSearch);
		} else
	...
	public void killLine() {
		...
		if (getDotOffset() < getDotLine().length()) {
			...
			fillToCaret(); // We might be beyond the end of the actual text on
							// the line.
			setMarkAtDot();
			dot.moveTo(dot.getNextLine(), 0);
		}
	...
	private void pasteInternal(String toBeInserted, boolean leavePasteSelected) {
		final Mode mode = buffer.getMode();
		CompoundEdit compoundEdit = beginCompoundEdit();
		if (mark == null && Utilities.isLinePaste(toBeInserted) && mode.acceptsLinePaste(this)
	...
	public void centerDialog(JDialog d) {
		...
		p.translate((parent.width - window.width) / 2, (parent.height - window.height) / 2);
		d.setLocation(p);
	}
			...
			Runnable r = new Runnable() {
				public void run() {
					try {
						...
						sb.append(command);
						sb.append('"');
						MessageDialog.showMessageDialog(Editor.this, sb.toString(), "Error");
					}
				}
			};
		...
		try {
			if (r == null) {
				...
				while (line != r.getEndLine()) {
					...
					dot.moveTo(line, 0);
					addUndo(SimpleEdit.LINE_EDIT);
					buffer.setIndentation(getDotLine(), buffer.getIndentation(getDotLine()) + amount);
					updateInAllEditors(getDotLine());
					line = line.next();
				}
				addUndo(SimpleEdit.MOVE);
	...
	public void dirDeleteFiles() {
		if (mark != null && getMarkLine() != getDotLine()) {
			MessageDialog.showMessageDialog(this, "This operation is not supported with multi-line text selections.",
					"Delete Files");
			return;
		}
		if (buffer instanceof Directory) {
			if (buffer.getFile() instanceof SshFile) {
				MessageDialog.showMessageDialog(this, "Deletions are not yet supported in ssh directory buffers.",
						"Error");
				return;
			}
	...
	/**
	 * Adds line to changed line list of all editors in which the current buffer
	 * is displayed.
	 *
	 * @param line
	 *            the line
	...
	public void updateHorizontalScrollBar() {
		if (horizontalScrollBar != null)
			horizontalScrollBar.setValues(display.getShift() * Display.getCharWidth(), display.getWidth(), 0,
	...
	private static void runStartupScript() {
		File file = File.getInstance(Directories.getEditorDirectory(), "init.lisp");
		...
		if (next != null) {
			...
			case PHP_MODE:
				...
				if (trim.startsWith("}")) {
					...
					Position end = new Position(line, line.getText().indexOf('}'));
				...
				if (next.trim().startsWith("}")) {
					...
					Position end = new Position(next, next.getText().indexOf('}'));
					Position start = findMatchInternal(end, 0);
					if (start != null) {
						foldNearLine(start.getLine());
						return;
			...
			case XML_MODE: {
				if (trim.startsWith("/>") || trim.startsWith("</")) {
					...
					while (prev != null && prev.isBlank())
						prev = prev.previous();
					if (prev != null) {
						int indent = buffer.getCol(line, line.getIndentation());
						int prevIndent = buffer.getCol(prev, prev.getIndentation());
					...
					int indent = buffer.getCol(line, line.getIndentation());
					int nextIndent = buffer.getCol(next, next.getIndentation());
	...
	private void fold(Line target) {
		...
		while (true) {
			...
			if (prev == null)
				break;
			if (prev.isBlank() || getMode().isCommentLine(prev) || isLabelLine(prev) || isPreprocessorLine(prev)) {
				begin = prev;
				continue;
			}
		...
		while (end != null) {
			if (end.isBlank() || getMode().isCommentLine(end) || isLabelLine(end) || isPreprocessorLine(end)) {
				end = end.next();
				continue;
			}
	...
	public void setEncoding() {
		File file = buffer.getFile();
		if (file != null) {
			InputDialog d = new InputDialog(this, "Encoding:", "Set Encoding", buffer.getSaveEncoding());
	...
	public void setEncoding(String encoding) {
		...
		if (file != null) {
			if (Utilities.isSupportedEncoding(encoding)) {
				...
				FastStringBuffer sb = new FastStringBuffer("Unsupported encoding \"");
				sb.append(encoding);
				sb.append('"');
