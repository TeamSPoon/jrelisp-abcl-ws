...
public final class DiffMode extends AbstractMode implements Constants, Mode {
	...
	private static void cvsGotoFile(Editor editor, DiffOutputBuffer diffOutputBuffer) {
		...
		if (text.startsWith("? ") || text.startsWith("Index: ")) {
			...
			File file = File.getInstance(diffOutputBuffer.getDirectory(), filename);
	...
	private static void darcsGotoFile(Editor editor, DiffOutputBuffer diffOutputBuffer) {
		final Line dotLine = editor.getDotLine();
		final int dotOffset = editor.getDotOffset();
		int lineNumber = 0;
		int context = 0;
	...
	private static void gitGotoFile(Editor editor, DiffOutputBuffer diffOutputBuffer) {
		...
		// }
		// }
		gotoLocation(editor, parentBuffer, lineNumber, dotOffset > 0 ? dotOffset - 1 : 0);
	}
	...
	private static void gotoLocation(Editor editor, Buffer buf, int lineNumber, int offset) {
		if (buf != null) {
			editor.makeNext(buf);
