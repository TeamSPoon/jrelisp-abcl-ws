...
public final class Display extends JComponent implements Constants, ActionListener, FocusListener {
	...
	public static void initializeStaticValues() {
		...
		changeMarkWidth = preferences.getIntegerProperty(Property.CHANGE_MARK_WIDTH);
	}
	...
	private final void providePaintLineImage(int width, int height) {
		if (paintLineImage != null && paintLineImageWidth == width && paintLineImageHeight == height)
			return;
	...
	private void paintImageLine(ImageLine imageLine, Graphics g, int y) {
		...
		g.drawImage(imageLine.getImage(), x, y, x + rect.width, y + rect.height, rect.x, rect.y, rect.x + rect.width,
	...
	private void drawGutterText(Graphics g, Line line, int y) {
		...
		if (c != 0) {
			...
			chars[0] = c;
			g.setColor(editor.getFormatter().getColor(0)); // Default text
															// color.
			g.setFont(plainFont);
	...
	private void drawText(Graphics2D g2d, char[] textArray, int length, int[] formatArray, int y) {
		int i = 0;
