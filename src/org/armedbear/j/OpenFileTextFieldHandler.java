...
public final class OpenFileTextFieldHandler extends DefaultTextFieldHandler implements Constants, MouseListener {
	...
	public void enter() {
		...
		if (value != null)
			entry = value;
		if (entry.startsWith("pop://") || entry.startsWith("{") || entry.startsWith("mailbox:")) {
				...
				if (Editor.getModeList().modeAccepts(IMAGE_MODE, file.getName())) {
					...
					if (modeId < 0 || modeId == HTML_MODE) {
						if (editor.getMode() instanceof WebMode) {
							...
							((WebBuffer) buf).saveHistory(buf.getFile(), buf.getAbsoluteOffset(editor.getDot()),
									((WebBuffer) buf).getContentType());
							// If we don't call setCache(null), go() will use
							// the
							// existing cache.
							buf.setCache(null);
	...
	public List getCompletions(String prefix) {
		...
		FilenameCompletion completion = new FilenameCompletion(dir, prefix, sourcePath, ignoreCase);
	...
	private void addCompletionsFromBufferList(List list, String prefix, File currentDirectory, boolean ignoreCase) {
		for (BufferIterator it = new BufferIterator(); it.hasNext();) {
			Buffer buf = it.nextBuffer();
		...
		if (Editor.preferences().getBooleanProperty(Property.SELECT_COMPLETION)) {
			if (originalText != null && originalText.length() > 0) {
				...
				if (select) {
					...
					if (c == '/' || c == '\\' || (Platform.isPlatformWindows() && originalText.length() >= 3
							&& originalText.charAt(1) == ':' && originalText.charAt(2) == '\\')) {
						final int index;
						if (ignoreCase) {
							index = completion.toLowerCase().lastIndexOf(originalText.toLowerCase());
						} else {
						...
						RE re = new UncheckedRE("[\\/]".concat(originalText), ignoreCase ? RE.REG_ICASE : 0);
	...
	private class CompletionsList extends JScrollPane implements MenuElement, MouseListener {
		...
		if (c >= ' ' && c != 127) {
			...
			if (textField.getSelectionStart() != textField.getSelectionEnd()) {
				if (originalText != null) {
					...
					FastStringBuffer sb = new FastStringBuffer(text.substring(0, textField.getSelectionStart()));
