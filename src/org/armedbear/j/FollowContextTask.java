 ...
 * $Id: FollowContextTask.java,v 1.1.1.1 2002/09/24 16:07:43 piso Exp $
 *
...
public class FollowContextTask extends IdleThreadTask implements Constants {
	...
	private final Runnable runnable = new Runnable() {
		public void run() {
			...
			Expression expression = editor.getMode().getExpressionAtDot(editor, false);
			if (expression == null)
				return;
	...
	private static Tag findMatchingTag(Editor editor, Expression expression) {
		List list = TagCommands.findMatchingTags(editor.getBuffer(), expression);
