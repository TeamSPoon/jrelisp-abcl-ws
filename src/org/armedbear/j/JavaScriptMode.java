...
public final class JavaScriptMode extends JavaMode implements Constants, Mode {
	...
	public int getCorrectIndentation(Line line, Buffer buffer) {
		...
		if (paren != null) {
			...
			if (paren.getLine().trim().endsWith("(") || !buffer.getBooleanProperty(Property.LINEUP_ARGLIST)) {
				indent = buffer.getIndentation(paren.getLine()) + indentSize;
			} else {
				paren.skip(1);
				while (paren.getOffset() < paren.getLineLength() && paren.getLine().charAt(paren.getOffset()) <= ' ') {
					paren.skip(1);
				}
		...
		final boolean indentAfterBrace = buffer.getBooleanProperty(Property.INDENT_AFTER_BRACE);
		...
		if (modelText.endsWith(")")) {
			...
			if (!firstToken.equals("if") && !firstToken.equals("else") && !firstToken.equals("for")
					&& !firstToken.equals("while") && !firstToken.equals("switch")) {
