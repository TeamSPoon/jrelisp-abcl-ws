...
public class Shell extends CommandInterpreter implements Constants {
	protected static final String JPTY_NOT_FOUND = "Unable to start shell process (jpty not found in PATH)";
	...
	private static final String getDefaultShellCommand() {
		String s = Editor.preferences().getStringProperty(Property.SHELL_FILE_NAME);
	...
	private static Shell createShell(String shellCommand) {
		...
		if (shell.getProcess() == null) {
			...
			String message = "Unable to start shell process \"" + shell.shellCommand + "\"";
	...
	protected void startProcess() {
		...
		Process p = null;
		try {
			p = Runtime.getRuntime().exec(cmdArray, null, new java.io.File(initialDir.canonicalPath()));
			setProcess(p);
		} catch (Throwable t) {
			setProcess(null);
			return;
		...
		if (Platform.isPlatformWindows() && shellCommand.toLowerCase().indexOf("cmd.exe") >= 0)
	...
	private void tab() {
		...
		final Completion completion = new Completion(currentDir, toBeCompleted, shellCommand);
