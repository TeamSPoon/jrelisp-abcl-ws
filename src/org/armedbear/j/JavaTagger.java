...
public class JavaTagger extends Tagger implements Constants {
	...
	private static final int CLASS_PROLOG = 3; // After name, before opening
												// brace.
	...
	private static final int FIELD_INITIALIZER = 10; // After '=' in a variable
														// declaration.
		...
		while (!pos.atEnd()) {
			...
			if (state == METHOD_NAME) {
				...
				if (pos.lookingAt("throws")) {
					addTag(TAG_METHOD);
					// Set token to null here so we don't try to add this method
					// again.
					token = null;
					visibility = 0;
					state = METHOD_PROLOG;
			...
			if (Character.isJavaIdentifierStart(c)) {
				...
				if (state == INTERFACE_NAME) {
					...
					tags.add(new JavaTag("interface ".concat(token), tokenStart, TAG_INTERFACE, visibility,
							parentClass));
					visibility = 0;
					...
					state = CLASS_PROLOG;
					// Add a tag for the class itself.
					tags.add(new JavaTag("class ".concat(token), tokenStart, TAG_CLASS, visibility, parentClass));
					visibility = 0;
				} else if (state == EXTENDS) {
					tags.add(new JavaTag(token, tokenStart, TAG_EXTENDS, visibility, currentClass));
					state = CLASS_PROLOG;
				} else if (state == IMPLEMENTS)
					tags.add(new JavaTag(token, tokenStart, TAG_IMPLEMENTS, visibility, currentClass));
					...
					// Don't be confused by lines like "Runnable r = new
					// Runnable() { ... };"
					state = NEW;
	...
	protected final LocalTag checkForExplicitTag(Position pos, int modeId) {
		final String explicitTag = Editor.preferences().getStringProperty(Property.EXPLICIT_TAG);
