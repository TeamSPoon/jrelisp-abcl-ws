...
public final class FindDialog extends AbstractDialog implements ActionListener, TextListener {
	...
	public FindDialog(Editor editor) {
		...
		regularExpressionCheckBox = new CheckBox("Regular expression", regularExpression);
		...
		if (Editor.checkExperimental()) {
			...
			multilinePatternCheckBox = new CheckBox("Multiline pattern (experimental)", isMultilinePattern);
		...
		listOccurrencesCheckBox = new CheckBox("List occurrences", listOccurrences);
		...
		searchFromStartCheckBox = new CheckBox("Search from start of document", searchFromStart);
