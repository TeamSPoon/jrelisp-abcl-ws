...
public final class ConfirmReplacementDialog extends AbstractDialog {
	...
	private void yes() {
		...
		if (replacement.restrictToSelection()) {
			...
			if (region != null) {
				if (pos.lineNumber() > region.getEndLineNumber() || (pos.getLine() == region.getEndLine()
			...
			if (replacement.restrictToSelection()) {
				...
				if (region != null) {
					if (editor.getDotLineNumber() > region.getEndLineNumber()
							|| (editor.getDotLine() == region.getEndLine() && editor.getDotOffset()
									+ replacement.getPatternLength() > region.getEndOffset())) {
						break;
					}
				}
			}
