...
public final class PHPFormatter extends Formatter implements Constants {
	...
	public boolean parseBuffer() {
		...
		changed = parseBufferHTML(new Position(line, 0), PHP_STATE_IGNORE, changed);
		buffer.setNeedsParsing(false);
		return changed;
	}

	private boolean parseBufferPHP(Position pos, int state, int htmlState, boolean changed) {
		char quoteChar = '\0';
		while (!pos.atEnd()) {
			char c = pos.getChar();
			if (c == EOL) {
	...
	private boolean parseBufferHTML(Position pos, int state, boolean changed) {
		while (!pos.atEnd()) {
			...
			case PHP_STATE_EQUALS:
				if (!Character.isWhitespace(c)) {
					if (c == '"')
						...
						pos.skip(5);
						changed = parseBufferPHP(pos, STATE_NEUTRAL, PHP_STATE_ATTNAME, changed);
						state = PHP_STATE_ATTNAME;
						continue;
					} else
						state = PHP_STATE_ATTVALUE;
			...
			case PHP_STATE_ATTVALUE_QUOTE:
				if (pos.lookingAt("<?php")) {
					pos.skip(5);
					changed = parseBufferPHP(pos, STATE_NEUTRAL, PHP_STATE_ATTVALUE_QUOTE, changed);
				} else {
					pos.next();
					if (c == '"')
						state = PHP_STATE_ATTNAME;
			...
			case PHP_STATE_ATTVALUE_SINGLEQUOTE:
				if (pos.lookingAt("<?php")) {
					pos.skip(5);
					changed = parseBufferPHP(pos, STATE_NEUTRAL, PHP_STATE_ATTVALUE_SINGLEQUOTE, changed);
				} else {
					pos.next();
					if (c == '\'')
						state = PHP_STATE_ATTNAME;
