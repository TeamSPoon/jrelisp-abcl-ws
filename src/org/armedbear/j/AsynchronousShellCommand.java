...
public final class AsynchronousShellCommand implements Constants, Runnable {
	...
	public AsynchronousShellCommand(String command, File directory, ShellCommandOutputBuffer buf) {
	...
	public void run() {
		...
		if (process != null) {
			ShellCommandReaderThread stdoutThread = new ShellCommandReaderThread(process.getInputStream());
			stdoutThread.start();
			ShellCommandReaderThread stderrThread = new ShellCommandReaderThread(process.getErrorStream());
			stderrThread.start();
	...
	private void killProcess() {
		if (processTable != null) {
			...
			if (entries != null && entries.size() > 0) {
				...
				ProcessTableEntry parent = (ProcessTableEntry) entries.get(entries.size() - 1);
				if (parent != null) {
					List children = processTable.findChildren(parent.pid);
					if (children != null) {
						for (int i = 0; i < children.size(); i++) {
							ProcessTableEntry entry = (ProcessTableEntry) children.get(i);
	...
	public static void startShellCommand(Editor editor, String command) {
		...
		AsynchronousShellCommand shellCommand = new AsynchronousShellCommand(command, dir, buf);
		shellCommand.start();
	}
}
