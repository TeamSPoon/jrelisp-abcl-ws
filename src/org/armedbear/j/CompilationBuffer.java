...
public final class CompilationBuffer extends CompilationErrorBuffer implements Runnable {
	...
	public void run() {
		...
		if (expandedCommand.startsWith("(")) {
			...
			sb.append(" ))");
			try {
				org.armedbear.lisp.LispObject result = JLisp.runLispCommand(sb.toString());
			...
			startProcess();
			if (process != null) {
				CompilationBufferReaderThread stdoutThread = new CompilationBufferReaderThread(
						process.getInputStream());
				stdoutThread.start();
				CompilationBufferReaderThread stderrThread = new CompilationBufferReaderThread(
