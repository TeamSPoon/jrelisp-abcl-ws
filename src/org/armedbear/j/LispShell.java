...
public class LispShell extends Shell {
	...
	private static final String ALLEGRO_PROMPT_PATTERN = "^(\\[[0-9+][ci]?\\] )?[^ ]+\\([0-9]+\\): ";

	private static final String ARMEDBEAR_PROMPT_PATTERN = ALLEGRO_PROMPT_PATTERN;

	private static final String CLISP_PROMPT_PATTERN = "^[^>\\*\\]]*\\[[0-9]+\\]> ";

	private static final String CMUCL_PROMPT_PATTERN = "^\\* |^[0-9]+\\] ";

	private static final String OPENMCL_PROMPT_PATTERN = "(^\\? )|(^Inspect ?[0-9]*\\>)";

	private static final String SBCL_PROMPT_PATTERN = CMUCL_PROMPT_PATTERN + "|" + ALLEGRO_PROMPT_PATTERN;
	...
	private static Shell createLispShell(String shellCommand, String title, boolean startSlime) {
		if (startSlime) {
			File portFile = File.getInstance(Directories.getEditorDirectory(), "swank");
			portFile.delete();
			if (shellCommand.indexOf("abcl") >= 0 || shellCommand.indexOf("org.armedbear.lisp") >= 0) {
				...
				if (swankLoader == null)
					return null; // FIXME Error message?
				if (shellCommand.indexOf("sbcl") >= 0 || shellCommand.endsWith("/xcl") || shellCommand.endsWith("\\xcl")
						|| shellCommand.endsWith("\\xcl.exe")) {
					shellCommand = shellCommand + " --load " + swankLoader.canonicalPath();
				} else if (shellCommand.indexOf("alisp") >= 0) {
					shellCommand = shellCommand + " -L " + swankLoader.canonicalPath();
				}
			}
		}
		...
		if (shellCommand.indexOf("alisp") >= 0) {
		...
		} else if (shellCommand.equals("/usr/bin/lisp") || shellCommand.equals("/usr/local/bin/lisp")) {
			...
			lisp.setExitCommand("(quit)");
		} else if (shellCommand.indexOf("org.armedbear.lisp") >= 0 || shellCommand.indexOf("abcl") >= 0) {
			lisp.setPromptRE(ARMEDBEAR_PROMPT_PATTERN);
			lisp.setResetCommand(":reset");
		} else if (shellCommand.indexOf("xcl") >= 0) {
			lisp.setPromptRE(ARMEDBEAR_PROMPT_PATTERN);
			lisp.setResetCommand("(ext:reset)");
		} else if (shellCommand.indexOf("openmcl") >= 0 || shellCommand.indexOf("lx86cl64") >= 0) {
	...
	protected void startProcess() {
		...
		Process p = null;
		try {
			p = Runtime.getRuntime().exec(cmdArray, null, new java.io.File(initialDirectory.canonicalPath()));
			setProcess(p);
		} catch (Throwable t) {
			setProcess(null);
			return;
	...
	public void enter() {
		...
		if (isComplete) {
			...
			if (lineBeforeLastPrompt != null) {
				posBeforeLastPrompt = new Position(lineBeforeLastPrompt, lineBeforeLastPrompt.length());
			}
		...
		Runnable r = new Runnable() {
			public void run() {
				...
				if (output.length() > 0) {
					appendString(output);
					if (match != null) {
						Line lineBeforeLastPrompt = getEnd().getLine().previous();
						if (lineBeforeLastPrompt != null) {
							posBeforeLastPrompt = new Position(lineBeforeLastPrompt, lineBeforeLastPrompt.length());
						}
					}
		...
		try {
			if (slime) {
				...
				if (posBeforeLastPrompt != null && posBeforeLastPrompt.getNextLine() != null) {
					...
					Position posLastPrompt = new Position(posBeforeLastPrompt.getNextLine(), 0);
	...
	private static final void _slime(String shellCommand, String title, boolean requireJpty) {
	...
	private static void lisp(String shellCommand, String title, boolean requireJpty, boolean startSlime) {
		...
		Runnable r = new Runnable() {
			public void run() {
				try {
					...
					JLisp.runLispCommand("(setq slime::*repl-buffer-name* \"" + buffer.getTitle() + "\")");
	...
	private static String getDefaultLispShellCommand() {
		...
		if (javaHome != null && javaHome.isDirectory()) {
			java = File.getInstance(javaHome, Platform.isPlatformWindows() ? "bin\\java.exe" : "bin/java");
		...
		if (classPath.indexOf(LocalFile.getPathSeparatorChar()) < 0) {
			...
			if (path.equals("j.jar") || path.endsWith("/j.jar") || path.endsWith("\\j.jar")) {
