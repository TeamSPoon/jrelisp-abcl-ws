...
public final class TagCommands implements Constants {
	...
	public static List findMatchingTags(Buffer buffer, Expression expression) {
		...
		if (list == null) {
			...
			list = findMatchingTagsInDirectory(expression, currentDirectory, mode);
			if (list == null) {
				...
				if (dirs != null) {
					for (int i = 0; i < dirs.size(); i++) {
						...
						if (directory.equals(currentDirectory))
							continue;
						List tagsInDir = findMatchingTagsInDirectory(expression, directory, mode);
						if (tagsInDir != null) {
							if (list == null)
								list = new ArrayList();
	...
	private static List findMatchingTagsInBuffer(Buffer buffer, Expression expression) {
	...
	public static List findMatchingTagsInDirectory(Expression expression, File directory, Mode mode) {
		if (!mode.isTaggable())
			return null;
	...
	public static List findMatchingTagsInDirectory(String name, File directory, Mode mode, int arity,
			boolean ignoreCase) {
		if (!mode.isTaggable())
			return null;
		List tags = Editor.getTagFileManager().getTags(directory, mode);
	...
	private static void listMatchingTags(Editor editor, String name) {
		if (name != null && name.length() > 0) {
			...
			List tags = findMatchingTagsInDirectory(name, buffer.getCurrentDirectory(), buffer.getMode(), -1,
