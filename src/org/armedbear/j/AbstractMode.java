 ...
 * $Id: AbstractMode.java,v 1.24 2005/11/17 20:46:33 piso Exp $
 *
...
public abstract class AbstractMode implements Constants, Mode {
	...
	protected AbstractMode(int id, String displayName) {
		...
		if (Editor.isLispInitialized()) {
			String hook = displayName.toLowerCase().replace(' ', '-') + "-mode-hook";
	...
	public synchronized final KeyMap getKeyMap() {
		if (keyMap == null) {
			if (Editor.isLispInitialized()) {
				String functionName = displayName.toLowerCase().replace(' ', '-').concat("-mode-map");
				FastStringBuffer sb = new FastStringBuffer("(ignore-errors (");
				sb.append("j::");
				sb.append(functionName);
				sb.append("))");
				try {
					LispObject result = Interpreter.evaluate(sb.toString());
	...
	protected ToolBar getCustomToolBar(Frame frame) {
		String filename = Editor.preferences().getStringProperty(getFullKey("toolbar"));
		if (filename != null) {
