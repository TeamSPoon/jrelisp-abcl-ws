...
public final class KeyMap implements Constants {
	...
	public static synchronized KeyMap getGlobalKeyMap() {
		if (globalKeyMap == null) {
			if (Editor.isLispInitialized()) {
				try {
					LispObject result = Interpreter.evaluate("(j:current-global-map)");
	...
	private void setGlobalDefaults() {
		...
		// Map these globally so they're available in the compilation buffer
		// too.
		...
		// Windows VM seems to need this mapping for the tab key to work
		// properly.
		// There's also code in Dispatcher.dispatchKeyTyped to handle the tab
		// key.
		mapKey(KeyEvent.VK_TAB, 0, "insertTab");

		if (Platform.isPlatformLinux()) {
			// These mappings work with Blackdown 1.2.2 (and 1.2 pre-release
			// v2).
	...
	public synchronized final KeyMapping lookup(KeyStroke keyStroke) {
		return lookup(keyStroke.getKeyChar(), keyStroke.getKeyCode(), keyStroke.getModifiers());
	}
