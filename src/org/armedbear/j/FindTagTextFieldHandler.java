...
public final class FindTagTextFieldHandler extends DefaultTextFieldHandler {
	...
	private static List findMatchingTags(Buffer buffer, String pattern) {
		...
		if (list == null) {
			...
			// directory.
			list = TagCommands.findMatchingTagsInDirectory(pattern, buffer.getCurrentDirectory(), mode, -1, ignoreCase);
			if (list == null) {
				...
				if (dirs != null) {
					for (int i = 0; i < dirs.size(); i++) {
						...
						if (directory.equals(buffer.getCurrentDirectory()))
							continue;
						List tagsInDir = TagCommands.findMatchingTagsInDirectory(pattern, directory, mode, -1,
								ignoreCase);
						if (tagsInDir != null) {
							if (list == null)
								list = new ArrayList();
	...
	private static List findMatchingTagsInBuffer(Buffer buffer, String pattern, boolean ignoreCase) {
	...
	public List getCompletions(final String prefix) {
		...
		List tags = Editor.getTagFileManager().getTags(editor.getCurrentDirectory(), mode);
		if (tags != null) {
