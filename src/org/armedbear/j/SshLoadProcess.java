...
public final class SshLoadProcess extends LoadProcess implements BackgroundProcess, Runnable, Cancellable {
	...
	private void _run() {
		...
		if (session.isDirectory(file.canonicalPath())) {
			...
			if (listing != null) {
				...
				// Report error!
				if (errorRunnable != null) {
					String message = "Unable to retrieve directory listing for ".concat(file.netPath());
