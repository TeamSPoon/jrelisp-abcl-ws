...
public class P4 extends VersionControl implements Constants {
	public static void p4() {
		if (!checkP4Installed())
			return;
		MessageDialog.showMessageDialog("The command \"p4\" requires an argument.", "Error");
	}
	...
	public static void p4(String s) {
		...
		Runnable commandRunnable = new Runnable() {
			public void run() {
				final String output = command(cmd, editor.getCurrentDirectory());
	...
	private static void p4Completed(Editor editor, Buffer parentBuffer, String cmd, String output) {
	...
	private static void editCompleted(Editor editor, Buffer buffer, String cmd, String output) {
	...
	public static void revert() {
		...
		if (buffer.isModified()) {
			String prompt = "Discard changes to " + maybeQuote(file.getName()) + "?";
	...
	public static void diff() {
		...
		if (parentBuffer.isModified()) {
			int response = ConfirmDialog.showConfirmDialogWithCancelButton(editor, CHECK_SAVE_PROMPT, "P4 diff");
			switch (response) {
			case RESPONSE_YES:
				save = true;
				break;
	...
	public static void log(String args) {
		...
		for (int i = 0; i < list.size(); i++) {
			...
			if (arg.charAt(0) != '-') {
				...
				File file = File.getInstance(parentBuffer.getCurrentDirectory(), arg);
	...
	public static void change(String arg) {
		...
		try {
			...
			MessageDialog.showMessageDialog("Argument must be a changelist number.", "Error");
		}
	}
	...
	private static void _change(String arg) {
		...
		final ShellCommand shellCommand = new ShellCommand(sb.toString(), parentBuffer.getCurrentDirectory());
		Runnable commandRunnable = new Runnable() {
			public void run() {
				shellCommand.run();
				Runnable completionRunnable = new Runnable() {
	...
	private static void _submit(String arg) {
		...
		if (list != null && list.size() > 0) {
			int response = ConfirmDialog.showConfirmDialogWithCancelButton(editor, "Save modified buffers first?",
					title);
			switch (response) {
			case RESPONSE_YES:
				save = true;
		...
		if (!save || saveModifiedBuffers(editor, list)) {
			...
			final CheckinBuffer checkinBuffer = new CheckinBuffer(parentBuffer, VC_P4);
			checkinBuffer.setProperty(Property.USE_TABS, true);
			checkinBuffer.setFormatter(new P4ChangelistFormatter(checkinBuffer));
			checkinBuffer.setTitle(title);
			checkinBuffer.setBusy(true);
	...
	public static void finish(final Editor editor, final CheckinBuffer checkinBuffer) {
		...
		final ShellCommand shellCommand = new ShellCommand(cmd, parentBuffer.getCurrentDirectory(), input);
		Runnable commandRunnable = new Runnable() {
			public void run() {
				shellCommand.run();
	...
	private static void finishCompleted(Editor editor, CheckinBuffer checkinBuffer, String title, boolean editOnly,
	...
	public static String getStatusString(File file) {
		...
		if (file != null && haveP4()) {
			...
			String output = command("p4 fstat ".concat(file.getName()), buffer.getCurrentDirectory());
	...
	private static boolean checkP4Installed() {
		if (haveP4())
			return true;
		MessageDialog.showMessageDialog("The Perforce command-line client does not appear to be in your PATH.",
				"Error");
		return false;
	}
