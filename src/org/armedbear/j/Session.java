...
public final class Session extends DefaultHandler implements Constants {
	...
	public static File getSessionDirectory() {
		if (sessionDirectory == null) {
			sessionDirectory = File.getInstance(Directories.getEditorDirectory(), "sessions");
	...
	public static void loadSession(String name) {
		...
		if (!session.load()) {
			...
			MessageDialog.showMessageDialog("Unable to load session from " + file, "Load Session");
			return;
		}
	...
	private Buffer createBuffers() {
		...
		Log.debug("createBuffers " + Editor.getBufferList().size() + " buffers " + elapsed + " ms");
	...
	public void save() {
		try {
			...
			BufferedWriter writer = new BufferedWriter(new OutputStreamWriter(tempFile.getOutputStream()));
