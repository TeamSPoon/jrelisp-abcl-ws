...
public final class Dispatcher implements Constants, KeyListener, MouseListener, MouseMotionListener, ActionListener,
	...
	public Dispatcher(Editor editor) {
		...
		DragGestureRecognizer dgr = dragSource.createDefaultDragGestureRecognizer(display,
	...
	private static Cursor getDragCursor(int index) {
		...
		if (url != null) {
			...
			if (image != null)
				cursor = toolkit.createCustomCursor(image, new Point(1, 1), name);
		}
		if (cursor == null)
