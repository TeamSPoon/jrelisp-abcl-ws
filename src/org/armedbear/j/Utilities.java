...
public final class Utilities implements Constants {
	...
	public static String detab(String s, int tabWidth, int startCol) {
		...
		// If we get here, we're looking at the first tab character in the
		// string.
	...
	public static String wrap(String s, int wrapCol, int tabWidth) {
		...
		while (i < limit) {
			...
			while (i < limit) {
				...
				if (c == ' ' || c == '\t') {
					// We've already incremented i to point past the whitespace
					// char.
					breakOffs = i;
					inUrl = false;
	 ...
	 * @param line
	 *            the #include line
	 * @return the name of the file, enclosed in quotes or angle brackets as the
	 *         case may be, or <code>null</code> if the line in question is not
	 *         a valid #include line.
	 * @since 0.16.2
	 */
	 ...
	 *
	 * @param s
	 *            the name of the file, enclosed in quotes or angle brackets as
	 *            the case may be
	 * @param path
	 *            the include path
	...
	private static final String filenameChars = "#-./0123456789:ABCDEFGHIJKLMNOPQRSTUVWXYZ\\_abcdefghijklmnopqrstuvwxyz~";
	...
	public static final String getEncodingFromCharset(String charset) {
		...
		if (lower.equals("unknown-8bit") || lower.equals("x-unknown") || lower.equals("us-ascii")
				|| lower.equals("default_charset") || lower.equals("latin-iso8859-1"))
			return "iso-8859-1";
		return charset;
	}
	...
	private static String[] keyNames = { "Enter", "Backspace", "Tab", "Escape", "Space", "Page Up", "Page Down", "Home",
			"End", "Delete", "Left", "Right", "Up", "Down", "NumPad Left", "NumPad Right", "NumPad Up", "NumPad Down",
			"NumPad *", "NumPad +", "NumPad -", "NumPad Insert", "Mouse-1", "Double Mouse-1", "Mouse-2",
			"Double Mouse-2", "Mouse-3", "Double Mouse-3" };

	private static int[] keyCodes = { KeyEvent.VK_ENTER, KeyEvent.VK_BACK_SPACE, KeyEvent.VK_TAB, KeyEvent.VK_ESCAPE,
			KeyEvent.VK_SPACE, KeyEvent.VK_PAGE_UP, KeyEvent.VK_PAGE_DOWN, KeyEvent.VK_HOME, KeyEvent.VK_END,
			KeyEvent.VK_DELETE, KeyEvent.VK_LEFT, KeyEvent.VK_RIGHT, KeyEvent.VK_UP, KeyEvent.VK_DOWN,
			KeyEvent.VK_KP_LEFT, KeyEvent.VK_KP_RIGHT, KeyEvent.VK_KP_UP, KeyEvent.VK_KP_DOWN, 0x6a, 0x6b, 0x6d, 0x9b,
			VK_MOUSE_1, VK_DOUBLE_MOUSE_1, VK_MOUSE_2, VK_DOUBLE_MOUSE_2, VK_MOUSE_3, VK_DOUBLE_MOUSE_3 };
	...
	static {
		if (Platform.isJava14()) {
			// Sun/Blackdown 1.4.x.
			defaultXMLReaderImpl = "org.apache.crimson.parser.XMLReaderImpl";
		} else {
			// 1.5
			defaultXMLReaderImpl = "com.sun.org.apache.xerces.internal.parsers.SAXParser";
		}
	}
