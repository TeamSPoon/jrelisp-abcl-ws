...
public final class FindInFiles extends Replacement implements Constants, BackgroundProcess {
	...
	private void replaceInFileNoConfirm(File file) throws SaveException {
		...
		if (buffer != null) {
			...
			// There was no buffer for the file in question prior to this
			// operation.
	...
	private void replaceInFileConfirm(File file) throws SaveException {
		...
		if (close) {
			if (buffer.isModified()) {
				buffer.writeBuffer(); // Throws SaveException if there's an
										// error.
				buffer.saved();
