...
public class FtpSession implements Constants {
	...
	int getFileStatus(String filename) {
		...
		if (code == 125 || code == 150 || code == 226) {
			...
			if (s != null && s.length() > 10) {
				char c = s.charAt(0);
				if (c == 'd')
					status = 2; // Shouldn't happen, since changeDirectory
								// failed.
				else if (c == '-')
					status = 1;
				if (status != -1) {
		...
		if (listing == null || listing.length() == 0) {
			...
			if (index >= 0) {
				...
				if (parentListing != null) {
					BufferedReader reader = new BufferedReader(new StringReader(parentListing));
					String entry;
					try {
	...
	private static boolean isEndOfReply(String s) {
		...
		if (s != null && s.length() >= 4 && s.charAt(3) == ' ' && Character.isDigit(s.charAt(0))
	...
	private boolean openDataSocket() {
		...
		if (usePassiveMode) {
			...
			while (true) {
				...
				// Otherwise the reply we received is not a reply to the PASV
				// command.
			...
			try {
				...
				String s = "PORT " + addr[0] + "," + addr[1] + "," + addr[2] + "," + addr[3] + "," + hibyte + ","
						+ lobyte;
				command(s);
				return getReply(200);
			} catch (Exception e) {
