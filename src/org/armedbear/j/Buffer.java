...
public class Buffer extends SystemBuffer {
	...
	private static Mode getModeForFileName(String filename) {
		...
		// Look for '/' on both Windows and Unix (filename might be a ZipEntry
		// name).
		...
		if (!isLoaded()) {
			...
			try {
				...
				if (toBeLoaded.isFile()) {
					...
					loadFile(toBeLoaded);
					// At this point, if we loaded from a cache, lastModified
					// will
	...
	private boolean saveLocalCompressed(File file) {
		...
		if (!maybeWriteBackupFromCache()) {
			FastStringBuffer sb = new FastStringBuffer("Unable to write backup file for ");
			sb.append(file.getName());
		...
		if (!saveToCache()) {
			FastStringBuffer sb = new FastStringBuffer("Unable to write temporary file for ");
			sb.append(file.getName());
	...
	private boolean compress(File source, File destination) {
		...
		try {
			...
			GZIPOutputStream out = new GZIPOutputStream(new BufferedOutputStream(tempFile.getOutputStream()), bufSize);
	 ...
	 *
	 * @param lineNumber
	 *            the original line number to look for
	 * @param offset
	...
	public Position getPosition(int goal) {
		...
		if (line == null)
			return null; // We hit the end of the buffer without reaching our
							// goal.
		if (offset == goal)
			return new Position(line, 0);
		offset -= line.length() + 1;
