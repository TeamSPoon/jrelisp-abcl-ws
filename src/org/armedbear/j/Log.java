 ...
 * $Id: Log.java,v 1.2 2003/06/29 00:19:34 piso Exp $
 *
...
public final class Log {
	...
	private static final SimpleDateFormat dateFormat = new SimpleDateFormat("MMM dd HH:mm:ss.SSS ");
	private static final int lineSeparatorLength = System.getProperty("line.separator").length();
	...
	private static final File getBackupLogFile(int index) {
		return File.getInstance(Directories.getEditorDirectory(), "log.".concat(String.valueOf(index)));
	}
	...
	private static final void loadPreferences() {
		synchronized (lock) {
			...
			maxFileSize = preferences.getIntegerProperty(Property.LOG_MAX_FILE_SIZE);
			if (maxFileSize < 10000)
				maxFileSize = 10000; // Minimum is 10 KB.
			else if (maxFileSize > 1000000)
				maxFileSize = 1000000; // Maximum is 1 MB.
			maxBackupIndex = preferences.getIntegerProperty(Property.LOG_MAX_BACKUP_INDEX);
			if (maxBackupIndex < 0)
				maxBackupIndex = 0; // Minimum is one backup.
		}
	}

	private static final PreferencesChangeListener preferencesChangeListener = new PreferencesChangeListener() {
