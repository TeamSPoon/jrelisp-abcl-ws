...
public final class InsertTagDialog extends InputDialog implements Constants {
	...
	public static void insertTag(Editor editor, String tagName, String extra, boolean wantEndTag) {
		if (extra == null)
			extra = "";
		...
		if (editor.getMark() != null)
			...
			if (offset > 0 && offset < dotLine.length()) {
				...
				if (!Character.isWhitespace(before) && !Character.isWhitespace(after)) {
					...
					if (begin != end)
						r = new Region(buffer, new Position(dotLine, begin), new Position(dotLine, end));
				}
			}
		}
		if (r != null) {
