...
public final class PropertiesDialog extends AbstractDialog implements Constants {
	...
	public PropertiesDialog() {
		...
		if (!buffer.isUntitled()) {
			...
			StaticTextField fileTextField = new StaticTextField(
					file.isRemote() ? file.netPath() : file.canonicalPath());
			...
			if (buffer.isReadOnly()) {
				if (file.getProtocol() == File.PROTOCOL_HTTP)
					...
					StaticTextField readOnlyTextField = new StaticTextField("File is read only on disk", WIDTH);
					group.add(Box.createVerticalStrut(6));
		...
		if (modeId != IMAGE_MODE && modeId != WEB_MODE) {
			...
			if (modeId != ARCHIVE_MODE && modeId != BINARY_MODE) {
				...
				case PERL_MODE:
				case TCL_MODE:
					indentBeforeBraceCheckBox = new CheckBox("Indent before '{'",
					...
					indentAfterBraceCheckBox = new CheckBox("Indent after '{'",
