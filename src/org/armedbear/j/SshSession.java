...
public final class SshSession implements Constants {
	...
	private static synchronized void unregister(SshSession session) {
		...
		Log.debug("leaving unregister() session count = " + sessionList.size());
	}
	...
	public boolean isDirectory(String canonicalPath) {
		SshFile file = new SshFile(hostName, canonicalPath, userName, null, port);
	...
	public synchronized boolean connect() {
		...
		try {
			...
			int timeout = Editor.preferences().getIntegerProperty(Property.SSH_TIMEOUT);
	...
	public RE getPromptRE() {
		if (_promptRE == null) {
			try {
				_promptRE = new RE(Editor.preferences().getStringProperty(Property.SSH_PROMPT_PATTERN));
			} catch (REException e) {
				Log.error(e);
	...
	private Runnable getPasswordRunnable = new Runnable() {
		public void run() {
			...
			password = PasswordDialog.showPasswordDialog(editor, passwordPrompt, passwordTitle);
	...
	private Runnable getPassphraseRunnable = new Runnable() {
		public void run() {
			final Editor editor = Editor.currentEditor();
			editor.setDefaultCursor();
			passphrase = PasswordDialog.showPasswordDialog(editor, passwordPrompt, passwordTitle);
			editor.setWaitCursor();
		}
	};
