...
public final class ReplaceDialog extends AbstractDialog implements Constants, TextListener {
	...
	public ReplaceDialog() {
		...
		replacement.setIgnoreCase(sessionProperties.getBooleanProperty(REPLACE_IGNORE_CASE, false));
		ignoreCaseCheckBox = new CheckBox("Ignore case", replacement.ignoreCase());
		ignoreCaseCheckBox.setMnemonic('I');
		addCheckBox(ignoreCaseCheckBox);
		replacement.setWholeWordsOnly(sessionProperties.getBooleanProperty(REPLACE_WHOLE_WORDS_ONLY, false));
		wholeWordsOnlyCheckBox = new CheckBox("Whole words only", replacement.wholeWordsOnly());
		wholeWordsOnlyCheckBox.setMnemonic('W');
		addCheckBox(wholeWordsOnlyCheckBox);
		replacement.setRegularExpression(sessionProperties.getBooleanProperty(REPLACE_REGULAR_EXPRESSION, false));
		regularExpressionCheckBox = new CheckBox("Regular expression", replacement.isRegularExpression());
		...
		if (Editor.checkExperimental()) {
			...
			if (replacement.isRegularExpression()) {
				replacement.setMultiline(sessionProperties.getBooleanProperty(REPLACE_MULTILINE_PATTERN, false));
			} else
				replacement.setMultiline(false);
			multilinePatternCheckBox = new CheckBox("Multiline pattern (experimental)",
		...
		restrictToSelectionCheckBox = new CheckBox("Restrict changes to selected text");
		restrictToSelectionCheckBox.setMnemonic('R');
		replacement.setConfirmChanges(sessionProperties.getBooleanProperty(REPLACE_CONFIRM_CHANGES, true));
		confirmChangesCheckBox = new CheckBox("Confirm changes", replacement.confirmChanges());
			...
			if (editor.getMark().getLine() == editor.getDot().getLine()) {
				...
				if (!regularExpressionCheckBox.isSelected()) {
					...
					if (pattern != null) {
						...
						// If the pattern is identical to the selected text, or
						// if
						// the selected text isn't long enough to contain a
						// match
						// for the pattern, it makes no sense to restrict
						// changes
	...
	protected void ok() {
		...
		SessionProperties sessionProperties = Editor.getSessionProperties();
		sessionProperties.setBooleanProperty(REPLACE_IGNORE_CASE, replacement.ignoreCase());
		sessionProperties.setBooleanProperty(REPLACE_WHOLE_WORDS_ONLY, replacement.wholeWordsOnly());
		sessionProperties.setBooleanProperty(REPLACE_REGULAR_EXPRESSION, replacement.isRegularExpression());
		sessionProperties.setBooleanProperty(REPLACE_MULTILINE_PATTERN, replacement.isMultilinePattern());
		sessionProperties.setBooleanProperty(REPLACE_CONFIRM_CHANGES, replacement.confirmChanges());
	...
	public static void replace() {
		...
		if (replacement.confirmChanges()) {
			...
			ConfirmReplacementDialog confirmDialog = new ConfirmReplacementDialog(replacement, false);
		...
		if (replacementCount == 0) {
			...
			FastStringBuffer sb = new FastStringBuffer(String.valueOf(replacementCount));
