...
public final class CompilationError {
	...
	private CompilationError(Line errorLine, String fileName, int lineNumber, int offset, String message) {
		...
		if (index >= 0) {
			...
			if (end >= 0) {
				...
				if (lineNumber > 0) {
					...
					if (message.length() == 0)
						message = null;
					return new CompilationError(line, fileName, lineNumber, -1, message);
				}
			}
			return null;
		}
		...
		if (index >= 0) {
			...
			if (index >= 0) {
				...
				if (lineNumber > 0) {
					...
					if (message.length() == 0)
						message = null;
					return new CompilationError(line, fileName, lineNumber, offset, message);
				}
			}
		}
		return null;
