...
public class LispMode extends AbstractMode implements Constants, Mode {
	...
	private static final boolean isSlimeLoaded() {
		if (Editor.isLispInitialized()) {
			try {
				LispObject result = Interpreter.evaluate("(ext:featurep :slime)");
	...
	private static final String validChars = "!$%&*+-./0123456789<=>?ABCDEFGHIJKLMNOPQRSTUVWXYZ[]^_abcdefghijklmnopqrstuvwxyz{}~";
		...
		if (offset > 0) {
			...
			if (c == ':') {
				...
				if (isIdentifierPart(c)) {
					...
					// by whitespace or maybe a '(', so it's a keyword. No
					// match.
					return false;
				}
			}
	...
	static {
		String[] strings = new String[] { "defclass", "defconstant", "defgeneric", "define-condition", "defmacro",
				"defmethod", "defparameter", "defstruct", "deftype", "defun", "defvar" };
	...
	private final String[] specials = new String[] { "block", "case", "catch", "do-all-symbols", "do-external-symbols",
			"do-symbols", "dolist", "dotimes", "ecase", "etypecase", "eval-when", "flet", "handler-bind", "labels",
			"lambda", "let", "let*", "locally", "loop", "macrolet", "multiple-value-bind", "multiple-value-prog1",
			"multiple-value-setq", "pprint-logical-block", "print-unreadable-object", "prog1", "prog2", "progn",
			"progv", "symbol-macrolet", "typecase", "unless", "when" };
	...
	public int getCorrectIndentation(Line line, Buffer buffer) {
		...
		if (posFirst != null) {
			...
			if (token.startsWith("def") || Utilities.isOneOf(token, specials) || Utilities.isOneOf(token, elispSpecials)
	...
	public static void compileLispFile() {
		...
		if (ed != null) {
			...
			boolean save = false;
			if (buffer.isModified()) {
				int response = ConfirmDialog.showConfirmDialogWithCancelButton(editor, CHECK_SAVE_PROMPT,
						"Compile File");
				switch (response) {
				case RESPONSE_YES:
					save = true;
