...
public final class WebBuffer extends Buffer implements WebConstants {
	...
	public static void followLink(boolean exact) {
		...
		if (destination instanceof HttpFile) {
			...
			final HttpLoadProcess httpLoadProcess = new HttpLoadProcess(wb, (HttpFile) destination);
				...
				public void run() {
					...
					if (isImage) {
						if (theLink instanceof ImageLink) {
							...
							// Normal link.
							// BUG!! This isn't right either. We should display
							// the image in
							// the current buffer.
							ImageBuffer buf = ImageBuffer.createImageBuffer(destination, httpLoadProcess.getCache(),
									null);
							editor.makeNext(buf);
							editor.activate(buf);
					...
					if (wb.loadLocalFile(httpLoadProcess.getCache(), contentType,
