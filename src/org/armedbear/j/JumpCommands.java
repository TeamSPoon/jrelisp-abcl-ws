...
public final class JumpCommands implements Constants {
	public static void jumpToLine() {
		...
		String response = InputDialog.showInputDialog(editor, "Line number:", "Jump To Line");
	...
	public static void jumpToColumn() {
		final Editor editor = Editor.currentEditor();
		String response = InputDialog.showInputDialog(editor, "Column number:", "Jump To Column");
		if (response == null || response.length() == 0)
			return;
		try {
	...
	public static void jumpToOffset() {
		final Editor editor = Editor.currentEditor();
		String response = InputDialog.showInputDialog(editor, "Offset:", "Jump To Offset");
		if (response == null || response.length() == 0)
			return;
		try {
