 ...
 * $Id: JDKHelp.java,v 1.1.1.1 2002/09/24 16:08:06 piso Exp $
 *
...
public final class JDKHelp implements Constants {
	...
	public static void jdkHelp(final String className) {
		if (className == null || className.length() == 0)
			return;
		final String jdkDocPath = Editor.preferences().getStringProperty(Property.JDK_DOC_PATH);
		if (jdkDocPath == null)
			return;
		...
		for (int i = 0; i < size; i++) {
			...
			File file = JavaSource.findImport(className, imports, dir, ".html");
			if (file == null || !file.isFile()) {
				...
				if (apiDir != null && apiDir.isDirectory()) {
					file = JavaSource.findImport(className, imports, apiDir, ".html");
				}
			}
			if (file != null && file.isFile()) {
				Buffer buf = null;
		...
		frame.setDefaultCursor();
		MessageDialog.showMessageDialog(editor, "No help available for ".concat(className), "JDK Help");
	}
	...
	public static void source(final String className) {
		...
		if (file != null) {
			...
			frame.setDefaultCursor();
			MessageDialog.showMessageDialog(editor, "No source available for ".concat(className), "Source");
		}
	}
	...
	private static String getClassNameAtPosition(Position pos) {
		...
		if (!Character.isJavaIdentifierPart(c) && c != '.')
			return null;
		// Go left until we encounter a character that can't be part of the
		// name.
		while (offset > 0) {
