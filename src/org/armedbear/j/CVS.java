...
public final class CVS extends VersionControl implements Constants {
	public static void cvs() {
		MessageDialog.showMessageDialog("The command \"cvs\" requires an argument", "Error");
	}
	...
	static void addCompleted(Editor editor, Buffer buffer, String cmd, String output) {
	...
	public static void commit() {
		...
		if (parentBuffer.getFile() == null)
			return;
		final String title = "cvs commit ".concat(parentBuffer.getFile().getName());
		boolean save = false;
		if (parentBuffer.isModified()) {
			int response = ConfirmDialog.showConfirmDialogWithCancelButton(editor, CHECK_SAVE_PROMPT, title);
			switch (response) {
			case RESPONSE_YES:
				save = true;
				break;
	...
	public static void diff(String args) {
		...
		boolean save = false;
		if (parentBuffer.isModified()) {
			int response = ConfirmDialog.showConfirmDialogWithCancelButton(editor, CHECK_SAVE_PROMPT, cmd);
			switch (response) {
			case RESPONSE_YES:
				save = true;
				break;
		...
		if (!save || parentBuffer.save()) {
			...
			Runnable commandRunnable = new Runnable() {
				public void run() {
					final String output = command(cmd, finalParentBuffer.getCurrentDirectory());
					Runnable completionRunnable = new Runnable() {
						public void run() {
							diffCompleted(editor, finalParentBuffer, cmd, output, VC_CVS);
						}
					};
					SwingUtilities.invokeLater(completionRunnable);
				}
	...
	public static void diffDir() {
		...
		final DiffOutputBuffer buf = new DiffOutputBuffer(directory, null, VC_CVS);
		buf.setTitle(cmd);
		editor.makeNext(buf);
	...
	public static void finish(final Editor editor, final CheckinBuffer checkinBuffer) {
		...
		if (!checkinBuffer.writeFile(tempFile)) {
			MessageDialog.showMessageDialog(editor, "Unable to write temporary file ".concat(tempFile.canonicalPath()),
					"Error");
			return;
		}
		...
		Runnable commandRunnable = new Runnable() {
			public void run() {
				final CvsCommand cvsCommand = new CvsCommand(cmd, parentBuffer.getCurrentDirectory());
				cvsCommand.run();
				tempFile.delete();
				Runnable completionRunnable = new Runnable() {
					public void run() {
						finishCompleted(editor, checkinBuffer, parentBuffer, cvsCommand, name, tempFile);
					}
				};
				SwingUtilities.invokeLater(completionRunnable);
			}
	...
	private static void finishCompleted(Editor editor, Buffer checkinBuffer, Buffer parentBuffer, CvsCommand cvsCommand,
	...
	// Implementation.
	static final String command(String cmd, File workingDirectory) {
