...
public final class JavaSource implements Constants {
	...
	public static File findSource(String className, String sourcePath) {
		...
		if (dirNames != null) {
			String fileName = className.replace('.', SEPARATOR_CHAR).concat(".java");
	...
	private static File findSource(Buffer buffer, String[] candidates) {
		...
		final String jdkSourcePath = buffer.getStringProperty(Property.JDK_SOURCE_PATH);
			...
			if (dirNames != null) {
				...
				for (Iterator it = dirNames.iterator(); it.hasNext();) {
					...
					if (dir == null) {
						...
						message = "The directory " + dir.canonicalPath() + " does not exist.";
					}
					if (message != null)
						MessageDialog.showMessageDialog(message, "Error");
				}
				for (int i = 0; i < candidates.length; i++) {
					File file = findImport(candidates[i], imports, dirNames, ".java");
					if (file != null)
						return file;
				}
			}
		...
		if (packageRootDir != null) {
			for (int i = 0; i < candidates.length; i++) {
				File file = findImport(candidates[i], imports, packageRootDir, ".java");
				if (file != null)
					return file;
			}
		}
		...
		for (int i = 0; i < candidates.length; i++) {
			File file = File.getInstance(currentDir, candidates[i].concat(".java"));
			if (file != null && file.isFile())
				return file;
		}
	...
	public static File getPackageRootDirectory(Buffer buffer) {
		...
		if (packageName == null)
			return null;
		final String packagePrefix = packageName.replace('.', LocalFile.getSeparatorChar());
		final String dirName = parentDir.canonicalPath();
		if (dirName.endsWith(packagePrefix)) {
			String packageRootDirName = dirName.substring(0, dirName.length() - packagePrefix.length());
	...
	private static File findImport(String className, String[] imports, List dirNames, String extension) {
	...
	// Returns null if file not found.
	public static File findImport(String className, String[] imports, File dir, String extension) {
		if (dir == null)
			return null;
		if (className.indexOf('.') >= 0) {
			// Canonical class name.
			String fileName = className.replace('.', SEPARATOR_CHAR).concat(extension);
			File file = File.getInstance(dir, fileName);
		...
		if (imports != null) {
			...
			for (int i = 0; i < imports.length; i++) {
				...
				if (s.endsWith(suffix)) {
					// Found it!
					String fileName = s.replace('.', SEPARATOR_CHAR).concat(extension);
					File file = File.getInstance(dir, fileName);
					return (file != null && file.isFile()) ? file : null;
				}
				if (s.endsWith(".*")) {
					...
					String filename = canonicalName.replace('.', SEPARATOR_CHAR).concat(extension);
					File file = File.getInstance(dir, filename);
					if (file != null && file.isFile())
						return file;
				}
