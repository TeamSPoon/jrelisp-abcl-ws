...
public final class ForthMode extends AbstractMode implements Constants, Mode {
	...
	public int getCorrectIndentation(Line line, Buffer buffer) {
		...
		String modelTrim = Utilities.detab(model.getText(), buffer.getTabWidth()).trim().toLowerCase();
		...
		if (modelTrim.equals("?do") || modelTrim.endsWith(" ?do"))
			return indented;
		String trim = Utilities.detab(line.getText(), buffer.getTabWidth()).trim().toLowerCase();
		index = trim.indexOf(" \\ ");
		if (index >= 0)
