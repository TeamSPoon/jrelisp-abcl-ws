 ...
 * $Id: WebLoader.java,v 1.3 2005/10/22 10:58:05 piso Exp $
 *
...
public final class WebLoader implements WebConstants {
	...
	private void processMarkup() throws EncodingChangeException {
		...
		if (tagName == "a") {
			if (link != null)
				// The last <a> tag was never terminated. This is probably a
				// typo for </a>.
				processEndAnchor();
			else
				processAnchor(tag);
					...
					if (c == '>' || Character.isWhitespace(c)) {
						...
						for (++i; i < limit; i++) {
							...
							if (c == ' ' || c == '>') {
								...
								--i;
								// We've already got the whole entity (if it is
								// one).
								break;
							}
							sbEntity.append(c);
	...
	private String gatherTag() {
		...
		try {
			while ((ch = reader.read()) >= 0) {
				char c = (char) ch;
				if (c == '<') {
					// We only expect to see a '<' inside a quoted attribute
					// value.
