...
public final class NewsGroupMessageBuffer extends MessageBuffer {
	...
	public NewsGroupMessageBuffer(NewsGroupSummary summary, NewsGroupSummaryEntry entry) {
		super();
	...
	public void nextArticle() {
		NewsGroupSummaryEntry nextEntry = (NewsGroupSummaryEntry) summary.getNextUndeleted(entry);
		if (nextEntry != null) {
			empty();
	...
	public void previousArticle() {
		NewsGroupSummaryEntry prevEntry = (NewsGroupSummaryEntry) summary.getPreviousUndeleted(entry);
		if (prevEntry != null) {
			empty();
			for (EditorIterator it = new EditorIterator(); it.hasNext();) {
				Editor ed = it.nextEditor();
	...
	protected void loadMessage(ProgressNotifier progressNotifier) {
		final String rawText = summary.getArticle(((NewsGroupSummaryEntry) entry).getArticleNumber(), progressNotifier);
		if (cancelled)
			return;
		...
		final MailboxLine mailboxLine = summary.findLineForEntry(entry);
		...
		try {
			...
			while ((s = reader.readLine()) != null) {
				...
				if (length >= 9 && s.startsWith("begin ") && haveUudecode()) {
					...
					File encoded = Utilities.getTempFile(Directories.getTempDirectory(), ".encoded");
					File decoded = Utilities.getTempFile(Directories.getTempDirectory(), extension);
					FastStringBuffer sb = new FastStringBuffer("begin 644 ");
					sb.append(decoded.getName());
					BufferedWriter writer = new BufferedWriter(
							new OutputStreamWriter(encoded.getOutputStream(), "ISO-8859-1"));
					...
					BufferedWriter writer = new BufferedWriter(
							new OutputStreamWriter(encoded.getOutputStream(), "ISO-8859-1"));
					FastStringBuffer sb = new FastStringBuffer(s.substring(0, index));
					sb.append(decoded.getName());
					writer.write(sb.toString());
					writer.write('\n');
					while ((s = reader.readLine()) != null) {
	...
	public void viewInline() {
		...
		for (; line != null; line = line.next()) {
			...
			if (s.startsWith("Inline: ")) {
				...
				File f = File.getInstance(Directories.getTempDirectory(), filename);
				if (f.isFile()) {
