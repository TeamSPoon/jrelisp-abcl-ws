 ...
 * $Id: LocalMailboxEntry.java,v 1.1.1.1 2002/09/24 16:10:12 piso Exp $
 *
...
public final class LocalMailboxEntry extends MailboxEntry {
	...
	public LocalMailboxEntry(int messageNumber, long messageStart, String s) {
		...
		from = MailAddress.parseAddresses(RFC2047.decode(headers.getValue(Headers.FROM)));
		replyTo = MailAddress.parseAddresses(RFC2047.decode(headers.getValue(Headers.REPLY_TO)));
		to = MailAddress.parseAddresses(RFC2047.decode(headers.getValue(Headers.TO)));
		cc = MailAddress.parseAddresses(RFC2047.decode(headers.getValue(Headers.CC)));
