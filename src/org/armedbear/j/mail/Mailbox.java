				...
				if (l instanceof MailboxLine) {
					...
					if (e.getDate().equals(entry.getDate())) {
						...
						if (RFC822Date.compare(e.getDate(), entry.getDate()) > 0) {
							// We're past it (assuming mailbox is sorted by
							// date).
							// The entry we're looking for was deleted. Return
							// the
							// current line.
							return line;
						}
	...
	public void foldThread(Line line) {
		...
		if (line instanceof MailboxLine) {
			...
			if (begin.next() instanceof MailboxLine) {
				...
				for (Line toBeHidden = begin.next(); toBeHidden != end
						&& toBeHidden != null; toBeHidden = toBeHidden.next())
					toBeHidden.hide();
				renumber();
	...
	private static void sortEntriesByDate(List list) {
		Comparator c = new Comparator() {
			public int compare(Object o1, Object o2) {
				return RFC822Date.compare(((MailboxEntry) o1).getDate(), ((MailboxEntry) o2).getDate());
			}
		};
