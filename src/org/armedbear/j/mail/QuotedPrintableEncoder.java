 ...
 * $Id: QuotedPrintableEncoder.java,v 1.1.1.1 2002/09/24 16:09:55 piso Exp $
 *
...
public final class QuotedPrintableEncoder {
	public static String encode(String input, String characterEncoding, String separator) {
		if (input.length() == 0)
			return input;
		byte[] bytes;
		try {
		...
		for (int i = 0; i < bytes.length; i++) {
			...
			if ((b < ' ' && b != '\t') || b == 127 || b == '=') {
				...
				if (i + 4 < bytes.length && bytes[i + 1] == 'r' && bytes[i + 2] == 'o' && bytes[i + 3] == 'm'
						&& bytes[i + 4] == ' ') {
