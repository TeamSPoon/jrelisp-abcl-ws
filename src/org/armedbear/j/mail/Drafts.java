...
public final class Drafts extends Mailbox {
	...
	public void delete() {
		...
		while (iter.hasNext()) {
			...
			File deleted = File.getInstance(directory, name.concat(".deleted"));
			if (deleted.isFile()) {
				Debug.bug();
				return;
			}
			Log.debug("delete renaming " + file.getName() + " to " + deleted.getName());
	...
	public void undelete() {
		...
		while (iter.hasNext()) {
			...
			File undeleted = File.getInstance(directory, name.substring(0, name.length() - 8));
			if (undeleted.isFile()) {
				Debug.bug();
				return;
			}
			Log.debug("undelete renaming " + file.getName() + " to " + undeleted.getName());
	...
	private void loadInternal() {
		...
		if (names != null) {
			for (int i = 0; i < names.length; i++) {
				final String name = names[i];
				DraftsEntry entry = DraftsEntry.parseEntry(directory, name);
				if (entry != null) {
					if (name.endsWith(".deleted"))
						entry.setFlags(entry.getFlags() | MailboxEntry.DELETED);
					entries.add(entry);
	...
	private static class DraftsEntry extends MailboxEntry {
		...
		public static DraftsEntry parseEntry(File directory, String fileName) {
			...
			if (file != null && file.isFile()) {
				...
				entry.subject = RFC2047.decode(headers.getValue(Headers.SUBJECT));
				...
				entry.replyTo = MailAddress.parseAddresses(RFC2047.decode(headers.getValue(Headers.REPLY_TO)));
				entry.to = MailAddress.parseAddresses(RFC2047.decode(headers.getValue(Headers.TO)));
				entry.cc = MailAddress.parseAddresses(RFC2047.decode(headers.getValue(Headers.CC)));
				entry.messageId = headers.getValue(Headers.MESSAGE_ID);
				entry.inReplyTo = parseInReplyTo(headers.getValue(Headers.IN_REPLY_TO));
