...
public final class NewsGroupSummary extends Mailbox {
			...
			if (count > 100) {
				Runnable confirmRunnable = new Runnable() {
					public void run() {
						...
						String response = InputDialog.showInputDialog(editor, prompt, groupName, defaultValue);
	...
	public void readArticle(Editor editor, Line line, boolean useOtherWindow) {
		if (line instanceof MailboxLine) {
			editor.setMark(null);
			NewsGroupSummaryEntry entry = (NewsGroupSummaryEntry) ((MailboxLine) line).getMailboxEntry();
		...
		if (text != null) {
			...
			try {
				...
				BufferedWriter writer = new BufferedWriter(
						new OutputStreamWriter(file.getOutputStream(), "ISO-8859-1"));
				String s;
				while ((s = reader.readLine()) != null) {
					writer.write(s);
					writer.write('\n');
		...
		if (session.writeLine("XOVER " + first + "-" + last)) {
			...
			if (s.startsWith("224")) {
				...
				while (true) {
					...
					if (s.equals("."))
						break;
					NewsGroupSummaryEntry entry = NewsGroupSummaryEntry.parseOverviewEntry(s);
					if (entry != null)
						entries.add(entry);
					++count;
