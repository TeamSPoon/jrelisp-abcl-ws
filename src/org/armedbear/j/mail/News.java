...
public final class News extends Buffer {
	private static final File newsDir = File.getInstance(Directories.getEditorDirectory(), "news");
			...
			if (session.connect()) {
				if (session.writeLine("LIST")) {
					...
					if (response.startsWith("215")) {
						...
						int count = 0;
						ProgressNotifier progressNotifier = new StatusBarProgressNotifier(this);
