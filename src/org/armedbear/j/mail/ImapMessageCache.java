...
public final class ImapMessageCache {
	...
	public void store(int uid, String message, String encoding) {
		...
		File file = File.getInstance(cacheDirectory, String.valueOf(uid));
		if (file == null)
			return;
		if (file.isFile()) {
		...
		try {
			...
			writer = new BufferedWriter(new OutputStreamWriter(file.getOutputStream(), encoding));
	...
	public String getMessageText(int uid) {
		try {
			...
			if (!file.isFile())
				return null;
			FastStringBuffer sb = new FastStringBuffer((int) (file.length() * 1.1));
			try {
				BufferedReader reader = new BufferedReader(new InputStreamReader(file.getInputStream()));
				String s;
			...
			sb.setLength(0);
			try {
				BufferedReader reader = new BufferedReader(new InputStreamReader(file.getInputStream(), encoding));
				String s;
				while ((s = reader.readLine()) != null) {
					sb.append(s);
					sb.append("\r\n");
	...
	public void removeDeletedEntries(List mailboxEntries) {
		...
		while (it.hasNext()) {
			...
			if (entry.isDeleted()) {
				File file = File.getInstance(cacheDirectory, String.valueOf(entry.getUid()));
	...
	private static synchronized File getCacheDirectory(ImapMailbox mb) {
		final File parentDirectory = File.getInstance(Directories.getMailDirectory(), "imap/cache");
			...
			while (keys.hasMoreElements()) {
				...
				if (key.indexOf('@') < 0 || key.indexOf(':') < 0) {
					...
					try {
						...
						File dir = File.getInstance(parentDirectory, value);
