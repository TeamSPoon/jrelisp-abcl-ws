...
public class MessageBuffer extends Buffer {
	...
	private static final String wrap(String s, int wrapCol, int tabWidth) {
		...
		while ((line = reader.readLine()) != null) {
			if (line.length() == 0) {
				...
				// Line is not empty.
				if (sb.length() == 0 && Utilities.getDetabbedLength(line, tabWidth) <= 78) {
			...
			public void run() {
				...
				if (succeeded) {
					...
					Runnable errorRunnable = new Runnable() {
						public void run() {
							MessageDialog.showMessageDialog(editor, "Failed", "Bounce Message");
						}
					};
		...
		for (Iterator it = names.iterator(); it.hasNext();) {
			...
			if (value != null) {
				if (name == "From" || name == "To" || name == "Cc") {
					MailAddress[] array = MailAddress.parseAddresses(RFC2047.decode(value));
					ArrayList list = new ArrayList();
					for (int i = 0; i < array.length; i++)
						list.add(array[i]);
					String prefix = Utilities.rightJustify(name, width).concat(": ");
					sb.append(MailUtilities.constructAddressHeader(prefix, list, prefix.length()));
				} else {
					// Subject, date.
			...
			try {
				while (true) {
					...
					if (text == null || text.length() == 0)
						break;
					if (maybeContinuation && Character.isWhitespace(text.charAt(0))) {
						sb.append(text);
						sb.append('\n');
	...
	private boolean isDefaultHeader(String s) {
		s = s.toLowerCase();
		if (s.startsWith("from:") || s.startsWith("to:") || s.startsWith("cc:") || s.startsWith("subject:")
				|| s.startsWith("date:"))
			return true;
		return false;
	}
	...
	public void viewAttachment() {
		...
		if (cache == null || !cache.isFile()) {
			MessageDialog.showMessageDialog("Unable to decode attachment", "View Attachment");
			return;
		}
	...
	public void saveAttachment() {
		...
		if (part == null)
			return;
		final Editor editor = Editor.currentEditor();
		SaveFileDialog d = new SaveFileDialog(editor, "Save Attachment", "File:");
		File suggested = File.getInstance(editor.getCurrentDirectory(), part.getAttachmentFileName());
		if (suggested != null)
		...
		if (!success)
			MessageDialog.showMessageDialog("Unable to save attachment", "Save Attachment");
	}
	...
	public void saveWindowState(Editor editor) {
		...
		if (otherEditor != null) {
			...
			float split = height / (editor.getFrame().getEditorPane().getHeight());
