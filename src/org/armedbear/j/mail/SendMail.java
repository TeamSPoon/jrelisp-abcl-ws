...
public final class SendMail extends Buffer {
	...
	public SendMail(MessageBuffer messageBuffer, boolean replyToGroup) {
		...
		try {
			...
			String s = messageBuffer.quoteBody(getIntegerProperty(Property.WRAP_COL));
			if (s != null)
				append(s);
			appendLine("");
			appendSignature();
		...
		try {
			...
			for (Line line = getFirstLine(); line != null; line = line.next()) {
				if (line.getText().equals(HEADER_SEPARATOR)) {
					...
					editor.addUndo(SimpleEdit.INSERT_STRING);
					insertString(editor.getDot(), MailUtilities.constructAddressHeader("Bcc: ", bccList).concat("\n"));
					break;
				}
			}
	...
	private void appendSignature() {
		...
		if (fileName != null)
			...
			file = File.getInstance(Directories.getUserHomeDirectory(), ".signature");
		...
		if (file != null && file.isFile()) {
			for (Line line = getFirstLine(); line != null; line = line.next()) {
				if (line.getText().equals(HEADER_SEPARATOR)) {
					...
					insertString(pos, "Attachment: " + file.canonicalPath() + "\n");
					break;
				}
			}
		}
		...
		Runnable sendRunnable = new Runnable() {
			public void run() {
				...
				if (session != null) {
					...
					try {
						OutputStreamWriter writer = new OutputStreamWriter(messageFile.getOutputStream());
						writeMessageText(writer);
						writer.flush();
						writer.close();
						succeeded = session.sendMessage(SendMail.this, messageFile);
						if (succeeded)
	...
	private boolean checkHeader() {
		...
		MessageDialog.showMessageDialog(Editor.currentEditor(), "Message separator line is missing", "Error");
		return false;
	...
	private void checkRecipients() {
		for (Line line = getFirstLine(); line != null; line = line.next()) {
			...
			if (lower.startsWith("to:") || lower.startsWith("cc:") || lower.startsWith("bcc:")) {
	...
	private Runnable errorRunnable = new Runnable() {
		public void run() {
			...
			MessageDialog.showMessageDialog(editor, session != null ? session.getErrorText() : "Unable to send message",
					"Send Mail");
		}
	};
	...
	private void writeMessageText(Writer writer) {
		...
		try {
			...
			String characterEncoding = getStringProperty(Property.DEFAULT_ENCODING);
			if (attachments != null) {
			...
			for (line = startOfBody; line != null; line = line.next()) {
				...
				if (qp) {
					writer.write(QuotedPrintableEncoder.encode(s, characterEncoding, separator));
				} else {
					// Dot stuffing.
	...
	private void removeHeaders(Editor editor, String hdr) {
		...
		while (true) {
			...
			Region r = new Region(this, new Position(beginLine, 0), new Position(endLine, 0));
			editor.addUndo(SimpleEdit.MOVE);
	...
	private void writeFcc(File messageFile) {
		...
		if (sentMessagesFile == null)
			return;
		try {
			BufferedReader reader = new BufferedReader(new InputStreamReader(messageFile.getInputStream()));
			BufferedWriter writer = new BufferedWriter(new FileWriter(sentMessagesFile.canonicalPath(), true));
			writer.write("From - ");
			SimpleDateFormat dateFormatter = new SimpleDateFormat("EEE MMM d HH:mm:ss yyyy");
	...
	private static final RE dateRE = new UncheckedRE("[A-Za-z]+, [0-9][0-9]? [A-Za-z]+ [0-9][0-9][0-9][0-9]");
	private static final RE timeRE = new UncheckedRE("[0-9:]+ ([+-][0-9][0-9][0-9][0-9]|[A-Z][A-Z][A-Z]+)");
		...
		for (int i = 0; i < limit; i++) {
			...
			if (c == '%' && ++i < limit) {
				...
				case 'd': {
					...
					if (m1 != null) {
						REMatch m2 = timeRE.getMatch(dateTime.substring(m1.getEndIndex()));
						if (m2 != null) {
				...
				default:
					Log.error("invalid format sequence \"%" + c + '"' + " in attribution");
					return null;
				}
			} else
				sb.append(c);
