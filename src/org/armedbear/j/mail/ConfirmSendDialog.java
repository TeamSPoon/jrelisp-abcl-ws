...
public final class ConfirmSendDialog extends AbstractDialog {
	...
	public ConfirmSendDialog(Editor editor, SendMail sm) {
		...
		addVerticalStrut();
		bccAddSenderCheckBox = new CheckBox("Bcc sender", sessionProperties.getBooleanProperty(bccAddSenderKey, false));
		bccAddSenderCheckBox.setMnemonic('S');
		bccAddSenderCheckBox.addKeyListener(this);
		mainPanel.add(bccAddSenderCheckBox);
		bccAddOtherCheckBox = new CheckBox("Bcc other:", sessionProperties.getBooleanProperty(bccAddOtherKey, false));
	...
	protected void ok() {
		...
		if (from.length() == 0 || from.indexOf('@') < 0) {
			MessageDialog.showMessageDialog("You must enter a valid \"From\" address", "Error");
			fromTextField.requestFocus();
			return;
		}
		if (bccAddOther) {
			if (bccOther.length() == 0 || bccOther.indexOf('@') < 0) {
				MessageDialog.showMessageDialog("You must enter a valid \"Bcc\" address", "Error");
				bccOtherTextField.requestFocus();
				return;
			}
		}
		if (smtp.length() == 0) {
			MessageDialog.showMessageDialog("You must specify the SMTP server", "Error");
	...
	public void keyPressed(KeyEvent e) {
		...
		KeyMapping mapping = editor.getKeyMapping(e.getKeyChar(), e.getKeyCode(), e.getModifiers());
