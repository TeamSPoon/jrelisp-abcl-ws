 ...
 * $Id: Mbox.java,v 1.1.1.1 2002/09/24 16:09:45 piso Exp $
 *
...
public final class Mbox {
	...
	public synchronized boolean appendMessage(Message message, final int flags) {
		...
		Debug.assertTrue(isLocked());
		try {
			BufferedReader reader = new BufferedReader(new StringReader(message.getRawText()));
			BufferedWriter writer = new BufferedWriter(new FileWriter(file.canonicalPath(), true));
			...
			while (true) {
				...
				if (s == null)
					return false; // Error! (Reached end of stream before
									// reaching end of headers.)
				if (s.length() == 0) {
