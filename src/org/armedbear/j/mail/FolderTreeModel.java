...
public final class FolderTreeModel extends DefaultTreeModel {
	...
	public static synchronized FolderTreeModel getDefaultModel() {
		if (model == null) {
			...
			File local = File.getInstance(Directories.getMailDirectory(), "local");
			if (local.isDirectory()) {
				DefaultMutableTreeNode localFolders = new DefaultMutableTreeNode("Local Folders");
				root.add(localFolders);
				// Add the drafts folder.
				Folder drafts = new Folder("drafts", new LocalMailboxURL(Directories.getDraftsFolder()));
				...
				for (int i = 0; i < list.length; i++) {
					...
					if (file.isDirectory()) {
						...
						final LocalMailboxURL url = new LocalMailboxURL(mailboxFile);
	...
	private void addNodeForFolder(MailboxURL url) {
		if (url instanceof ImapURL) {
			...
			for (int i = 0; i < list.size() - 1; i++) {
				...
				if (add) {
					DefaultMutableTreeNode node = new DefaultMutableTreeNode(list.get(i));
					parent.add(node);
					parent = node;
				}
			}
