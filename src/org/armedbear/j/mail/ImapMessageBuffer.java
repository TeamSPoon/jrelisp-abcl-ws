...
public final class ImapMessageBuffer extends MessageBuffer {
	...
	/* package */ ImapMessageBuffer(ImapMailbox mailbox, ImapMailboxEntry entry, String rawText) {
		super();
		init(mailbox, entry);
	...
	private BackgroundProcess loadProcess = new BackgroundProcess() {
		...
		public void run() {
			...
			try {
				...
				progressNotifier = new StatusBarProgressNotifier(ImapMessageBuffer.this);
	...
	public void moveMessage() {
		...
		if (!s.startsWith("mailbox:")) {
			...
			if (s == null) {
				MessageDialog.showMessageDialog(editor, "Invalid destination", "Error");
				return;
			}
		}
		if (!mailbox.lock()) {
