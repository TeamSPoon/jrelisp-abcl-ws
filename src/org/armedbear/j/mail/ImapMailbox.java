...
public final class ImapMailbox extends Mailbox {
		...
		public void run() {
			...
			try {
				...
				if (!session.verifySelected(folderName)) {
					Log.error("GetNewMessagesProcess.run can't select " + folderName);
					return;
				}
				if (cancelled) {
					Log.debug("cancelled, disconnecting...");
				...
				if (session.isReadOnly()) {
					Log.warn("GetNewMessagesProcess.run " + folderName + " is read-only, returning...");
					return;
				}
				if (uidValidity != session.getUidValidity()) {
					...
					if (session.getMessageCount() != messageCount) {
						Log.debug("session.getMessageCount() = " + session.getMessageCount()
								+ " mailbox message count = " + messageCount);
	...
	public void createFolder() {
		...
		final String input = InputDialog.showInputDialog(editor, "Folder:", "Create Folder");
	...
	public void deleteFolder() {
		final Editor editor = Editor.currentEditor();
		final String input = InputDialog.showInputDialog(editor, "Folder:", "Delete Folder");
		if (input == null || input.length() == 0)
			return;
		final String name = extractFolderName(input);
		if (name == null)
	...
	public void saveToFolder() {
		...
		if (count > 1)
			sb.append('s');
		sb.append(" to:");
		final String destination = ChooseFolderDialog.chooseFolder(editor, sb.toString(), title);
		if (destination == null)
			return;
	...
	private boolean saveLocal(List toBeSaved, String destination, boolean delete) {
		...
		for (int i = 0; i < toBeSaved.size(); i++) {
			...
			if (message != null && mbox.appendMessage(message, entry.getFlags() & ~MailboxEntry.TAGGED)) {
				if (delete) {
	...
	private List retrieveMessageHeaders(int uidBegin, int uidEnd, boolean recent) {
		Log.debug("retrieveMessageHeaders " + folderName + " " + uidBegin + " " + uidEnd);
		...
		StatusBarProgressNotifier progressNotifier = new StatusBarProgressNotifier(this);
	...
	private void addEntry(List list, String s, int uidBegin, boolean recent) {
		...
		if (entry != null) {
			if (entry.getUid() >= uidBegin) {
				...
				Log.debug("not adding message " + entry.getMessageNumber() + " uid " + entry.getUid() + " uidBegin = "
						+ uidBegin);
				if (messageCount < 0) {
	...
	public void readOnlyError() {
		Runnable r = new Runnable() {
			public void run() {
				MessageDialog.showMessageDialog("Mailbox is read-only", "Error");
			}
		};
		SwingUtilities.invokeLater(r);
	}
	...
	private void fatal(final String text, final String title) {
		Runnable r = new Runnable() {
			public void run() {
				MessageDialog.showMessageDialog(Editor.currentEditor(), text, title);
				if (Editor.getBufferList().contains(ImapMailbox.this))
					kill();
			}
		};
	...
	private boolean getAllMessageHeaders() {
		...
		t.start();
		if (!session.verifyConnected()) {
			Log.error("getAllMessageHeaders can't connect to " + session.getHost() + " on port " + session.getPort());
			setBusy(false);
			if (!cancelled)
		...
		if (!session.verifySelected(folderName)) {
			Log.error("getAllMessageHeaders can't SELECT " + folderName + " on " + session.getHost());
			setBusy(false);
			if (!cancelled)
				fatal(session.getErrorText(), "Error");
			return false;
	...
	private void updateCachedEntries(List cachedEntries) {
		...
		while (true) {
			...
			ImapMailboxEntry entry = (ImapMailboxEntry) map.get(new Integer(uid));
			if (entry != null) {
		...
		long elapsed = System.currentTimeMillis() - start;
		Log.debug("updateCachedEntries " + elapsed + " ms " + (float) elapsed / cachedEntries.size() + " ms per entry");
	}
	...
	private void readMessage(Line line, boolean useOtherWindow) {
		...
		ImapMailboxEntry entry = (ImapMailboxEntry) ((MailboxLine) line).getMailboxEntry();
		Buffer buf = null;
		...
		if (buf != null) {
			...
			activateMessageBuffer(editor, (ImapMessageBuffer) buf, useOtherWindow);
			return;
		}
		if (getBooleanProperty(Property.IMAP_USE_LOCAL_CACHE)) {
			String rawText = getMessageTextFromCache(entry.getUid());
			if (rawText != null) {
				ImapMessageBuffer mb = new ImapMessageBuffer(this, entry, rawText);
	...
	private String fetchPart(int uid, String part, String encoding, ProgressNotifier progressNotifier) {
	...
	public void dispose() {
		Log.debug("ImapMailbox.dispose " + folderName + " on " + session.getHost());
		Runnable r = new Runnable() {
			public void run() {
				session.logout();
			}
	...
	protected void finalize() throws Throwable {
		Log.debug("ImapMailbox.finalize " + folderName + " on " + session.getHost());
		super.finalize();
	}
