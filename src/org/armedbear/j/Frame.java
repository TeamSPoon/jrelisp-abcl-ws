...
public final class Frame extends JFrame
		implements Constants, ComponentListener, FocusListener, WindowListener, WindowStateListener {
	...
	public void titleChanged() {
		FastStringBuffer sb = new FastStringBuffer(Version.getShortVersionString());
	...
	public void splitWindow() {
		if (editors[1] == null) {
			...
			editors[1].updateLocation();
			SplitPane sp = new SplitPane(SplitPane.VERTICAL_SPLIT, editors[0], editors[1]);
			sp.setBorder(null);
	...
	public void switchToBuffer(final Buffer buf) {
		Debug.bugIfNot(buf.isPaired() || (getEditorCount() == 2 && editors[0].getBuffer().isPaired()));
		...
		if (getEditorCount() == 2) {
			...
			if (secondary != null) {
				...
				if (editorPane instanceof SplitPane) {
					...
					int dividerLocation = (int) (height * (1 - split) - sp.getDividerSize());
				...
				if (b != null) {
					b.autosave();
					kill.saveView();
					RecentFiles.getInstance().bufferDeactivated(b, kill.getDot());
					b.windowClosing();
				}
	...
	public void setWindowHeight(Editor editor, int n) {
		if (editorPane instanceof SplitPane) {
			...
			int minHeightForOtherWindow = otherEditor.getLocationBarHeight() + charHeight * 4 + scrollBarHeight;
			int availableHeight = sp.getHeight() - minHeightForOtherWindow - sp.getDividerSize();
			int requestedHeight = editor.getLocationBarHeight() + charHeight * n + scrollBarHeight;
	...
	private Editor openInOtherWindow(Editor editor, Buffer buffer, float split, boolean switchWindows) {
		...
		if (editors[1] == null) {
			...
			editors[1].updateLocation();
			SplitPane sp = new SplitPane(SplitPane.VERTICAL_SPLIT, editor, editors[1]);
			sp.setBorder(null);
			int dividerLocation = (int) (editor.getHeight() * (1 - split) - sp.getDividerSize());
			sp.setDividerLocation(dividerLocation);
			editorPane = sp;
			if (sidebar != null) {
				sidebarSplitPane.setRightComponent(editorPane);
				sidebarSplitPane.setDividerLocation(Editor.getSessionProperties().getSidebarWidth(this));
			} else {
				// No sidebar.
