...
public class Search implements Cloneable {
	...
	private Position findMultilineRegExp(Buffer buffer, Position start) {
		...
		while (true) {
			...
			if (!wholeWordsOnly)
				break;
			if (Utilities.isDelimited(buffer.getMode(), s, match.getStartIndex(), match.getEndIndex()))
				break;
	...
	private Position reverseFindMultilineRegExp(Buffer buffer, Position start) {
		...
		while (true) {
			...
			if (!wholeWordsOnly)
				lastMatch = match;
			else if (Utilities.isDelimited(buffer.getMode(), s, match.getStartIndex(), match.getEndIndex()))
				lastMatch = match;
			startIndex = match.getStartIndex() + 1;
		}
		if (lastMatch == null)
	...
	private boolean findRegExp(String toBeSearched) {
		...
		while (index <= limit) {
			...
			if (match == null)
				break;
			if (!wholeWordsOnly
					|| Utilities.isDelimited(toBeSearched, match.getStartIndex(), match.toString().length()))
				return true;
			index = match.getStartIndex() + 1;
		}
		return false;
