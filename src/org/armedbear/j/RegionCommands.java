...
public final class RegionCommands {
	...
	private static void detabOrEntabRegion(Editor editor, Buffer buffer, Region r, boolean entab, int tabWidth) {
		...
		if (beginLine == endLine) {
			...
			final String toBeChanged = oldText.substring(beginOffset, endOffset);
	...
	public static void renumberRegion(String arg) {
		...
		if (arg != null) {
			try {
				...
				MessageDialog.showMessageDialog("Invalid number \"" + arg + '"', "Error");
				return;
			}
		}
		final Buffer buffer = editor.getBuffer();
	...
	private static void _renumberRegion(Editor editor, Buffer buffer, Region region, int start) {
	...
	public static void doShellCommandOnRegion() {
		...
		if (!editor.checkReadOnly())
			return;
		if (editor.getMark() == null) {
			MessageDialog.showMessageDialog(editor, "No region selected", "Error");
			return;
		}
		InputDialog d = new InputDialog(editor, "Command:", "Do Shell Command On Region", null);
		...
		// it's big.
		ShellCommand shellCommand = new ShellCommand(command, null, r.toString());
		shellCommand.run();
