...
public final class Macro implements Constants {
	...
	public static synchronized void recordMacro() {
		if (Editor.isRecordingMacro())
			...
			if (macro != null && !macro.isEmpty()) {
				if (!editor.confirm("Record Macro", "Overwrite existing keyboard macro?"))
					return;
			}
	...
	public static synchronized void startMacro() {
		if (Editor.isRecordingMacro()) {
			MessageDialog.showMessageDialog(Editor.currentEditor(), "Command ignored (already recording a macro)",
					"Start Macro");
		} else {
			final Editor editor = Editor.currentEditor();
			if (macro != null && !macro.isEmpty()) {
				if (!editor.confirm("Start Macro", "Overwrite existing keyboard macro?"))
					return;
			}
			macro = new Macro(editor);
			Editor.setRecordingMacro(true);
	...
	public static synchronized void endMacro() {
		if (Editor.isRecordingMacro()) {
			...
			MessageDialog.showMessageDialog(Editor.currentEditor(), "Command ignored (not recording a macro)",
					"End Macro");
		}
	}
	...
	public static synchronized void playbackMacro() {
		...
		if (Editor.isRecordingMacro()) {
			MessageDialog.showMessageDialog(editor,
					"Command ignored (playbackMacro is not allowed while recording a macro)", "Record Macro");
			return;
		}
		if (macro == null || macro.isEmpty()) {
	...
	public static synchronized void record(Editor editor, Object command) {
		if (command == "recordMacro" || command == "playbackMacro" || command == "startMacro" || command == "endMacro")
			return;
