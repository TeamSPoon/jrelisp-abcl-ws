...
public final class Directories {
	...
	public static void initialize(File userHome) {
		...
		if (userHomeDirectory == null) {
			...
			if (Platform.isPlatformWindows()) {
				...
				// Not Windows.
				userHomeDirectory = File.getInstance(System.getProperty("user.home"));
			}
			if (userHomeDirectory == null)
				Editor.fatal("Use \"--home\" option to specify home directory.");
		}
		// Make sure required directories exist and are writable.
		editorDirectory = provideDirectory(File.getInstance(userHomeDirectory, ".j"));
		tempDirectory = provideDirectory(File.getInstance(editorDirectory, "temp"));
		mailDirectory = provideDirectory(File.getInstance(editorDirectory, "mail"));
		draftsFolder = provideDirectory(File.getInstance(mailDirectory, "local/drafts"));
		registersDirectory = provideDirectory(File.getInstance(editorDirectory, "registers"));
	...
	public static final void moveUnsentMessagesToDraftsFolder() {
		File unsentMessagesDirectory = File.getInstance(mailDirectory, "unsent");
		...
		for (int i = 0; i < files.length; i++) {
			File source = File.getInstance(unsentMessagesDirectory, files[i]);
			File destination = File.getInstance(draftsFolder, files[i]);
		...
		if (files.length == 0) {
			...
			Log.debug("not removing directory " + unsentMessagesDirectory + " (directory is not empty)");
	}
}
