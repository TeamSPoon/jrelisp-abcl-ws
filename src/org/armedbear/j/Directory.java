...
public final class Directory extends Buffer {
	...
	static {
		...
		nativeMoveToFilenameRegExp = new UncheckedRE("(" + traditional + ")|(" + iso + ")|(" + osx + ")", 0, syntax);
	...
	public synchronized void reload() {
		...
		for (int i = 0; i < editors.size(); i++) {
			...
			if (ed.getBuffer() == this) {
				...
				if (dotLine == null) {
					dotLine = getLine(((Integer) lineNumbers.get(i)).intValue());
					if (dotLine == null) {
				...
				Line line = getLine(((Integer) topLineNumbers.get(i)).intValue());
				if (line == null) {
	...
	private void addEntriesToBuffer() {
		...
		if (preferences.getBooleanProperty(Property.DIR_SORT_DIRECTORIES_FIRST, !usingNativeFormat)) {
			// Add lines to the buffer in two passes so directories will always
			// be on top.
	...
	private synchronized void loadInternal() {
		...
		boolean useNativeFormat = preferences.getBooleanProperty(Property.DIR_USE_NATIVE_FORMAT,
		...
		if (dir.isRemote()) {
			...
			if (isDirectory) {
				...
				if (newFile instanceof FtpFile)
					buf = new RemoteBuffer((FtpFile) newFile, FtpSession.getSession((FtpFile) newFile));
	...
	public void browseFileAtDot() {
		...
		if (f.isFile()) {
			...
			try {
				...
				String browserOpts = preferences.getStringProperty(Property.BROWSER_OPTS);
				if (browserOpts != null) {
	...
	public static void dirDoShellCommand() {
		...
		if (buffer instanceof Directory) {
			if (buffer.getFile().isRemote()) {
				MessageDialog.showMessageDialog(editor, "Shell commands are not supported in remote directory buffers.",
						"Error");
				return;
			}
