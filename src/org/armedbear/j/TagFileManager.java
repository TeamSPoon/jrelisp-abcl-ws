...
public final class TagFileManager extends Thread {
	...
	public TagFileManager() {
		...
		setDaemon(true);
		tagFileDir = File.getInstance(Directories.getEditorDirectory(), "tagfiles");
	...
	public void makeTagFile(File dir, Mode mode) {
		...
		try {
			...
			if (tagfile != null) {
				...
				if (files != null) {
					BufferedWriter writer = new BufferedWriter(new OutputStreamWriter(tagfile.getOutputStream()));
	...
	public List getTags(File directory, Mode mode) {
		...
		if (tags == null) {
			try {
				BufferedReader reader = new BufferedReader(new InputStreamReader(tagFile.getInputStream()));
				...
				if (s != null && s.equals(VERSION)) {
					...
					Log.warn("getTags wrong version " + directory + " " + mode);
				}
				reader.close();
			} catch (IOException e) {
				Log.error(e);
	...
	private static class TagFileCache {
		...
		synchronized void add(File directory, String modeName, File tagFile, List tags) {
	...
	private static class CacheEntry {
		...
		CacheEntry(File directory, String modeName, File tagFile, List tags) {
			this.directory = directory;
		...
		public String toString() {
			return directory.canonicalPath() + " " + modeName + " " + String.valueOf(lastAccess);
		}
	}
}
