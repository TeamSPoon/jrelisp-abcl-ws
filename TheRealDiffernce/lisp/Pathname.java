...
import org.armedbear.lisp.io.IPathname;
import org.armedbear.lisp.io.InputStreamFacade;

public class Pathname extends LispObject implements IPathname {

	private static Object jarFileFactory;
	...
	static {
		try {
			...
			Constructor<?> c = jarFileFactoryClass.getDeclaredConstructor();
			c.setAccessible(true);
			jarFileFactory = c.newInstance();
			jarFileFactory_getCachedJarFile = jarFileFactoryClass.getDeclaredMethod("getCachedJarFile", URL.class);
			jarFileFactory_getCachedJarFile.setAccessible(true);
			jarFileFactory_close = jarFileFactoryClass.getDeclaredMethod("close", JarFile.class);
			jarFileFactory_close.setAccessible(true);
    ...
    private static class remove_zip_cache_entry extends Primitive { 
        ...
        public LispObject execute(LispObject arg) {
            ...
            boolean result = p.removeZipCacheEntry();
            return result ? T : NIL;
        }
    }
    
    
    public boolean removeZipCacheEntry() {
    	
    	URL url = toURL();
    	JarFile f;
    	
    	try {
			f = (JarFile)jarFileFactory_getCachedJarFile.invoke(jarFileFactory, url);
			
			if (f == null) {
				return false;
			}
			
			jarFileFactory_close.invoke(jarFileFactory,f);
...
//	}
	...
	private static class pf_match_wild_jar_pathname extends Primitive {
		...
		public LispObject execute(LispObject arg) {
			...
			if (pathname.device.cdr() instanceof Cons) {
				...
				ZipEntry entry = null;
				try {
					oentry = outerJar.getNextEntry();
				} catch (IOException e) {
					oentry = null;
				}
				
				while(oentry != null && entry == null) {
					if (oentry.getName().equals(entryPath)) {
						entry = oentry;
						oentry = null;
					} else {
						try {
				...
				byte[] odata = new byte[(int)entry.getSize()];
				
				
				InputStream inputStream = null;
				//InputStream inputStream = new ByteArrayInputStream(odata);
				try {
					outerJar.read(odata);
					inputStream = new java.io.ByteArrayInputStream(odata);
											...
											boolean errorIfDoesNotExist) 
	{
		...
		if (!(pathname.isJar() || pathname.isURL())) {
			...
			if ((tmp_is = pathname.getInputStream()) != null) {
				try {
					tmp_is.close();
				} catch (IOException e) {
				}
				...
				if (pathname.name != NIL 
				  ...
				  && Symbol.GETF.execute(pathname.host, FRAGMENT, NIL) == NIL) {
					...
					if ((tmp_is =p.getInputStream()) != null) {
						try {
							tmp_is.close();
						} catch (IOException e) {
						}
			...
			try {
				...
				if (null != is) {
					...
					if (!normalURLString.endsWith("/")) {
						...
						Pathname dpath = new Pathname(qpath.toURL().toString() + "/");
						
						isd = dpath.getInputStream();
						if (isd != null) {
							try {
								isd.close();
							} catch (Throwable t) {
  ...
  private static final class pf_get_input_stream extends Primitive {
	...
	{
		InputStreamFacade is =  new InputStreamFacade(this);
		...
		if (isJar()) {
			...
			if (d.cdr().equals(NIL)) {
				if (entryPath.length() == 0) {
					...
					ZipInputStream jar = new ZipInputStream(jaris);
					
					ZipEntry ientry = null;
					ZipEntry entry = null;
					
					try {
						ientry = jar.getNextEntry();
					} catch (IOException e) {
						ientry = null;
					}
					
					while(ientry != null && entry == null) {
						
						if (ientry.getName().equals(entryPath)) {
							entry = ientry;
						}
				...
				if (entryPath.length() == 0) {
					...
					try {
						ientry = outerJar.getNextEntry();
					} catch (IOException e) {
					}
					
					while(entry == null && ientry != null) {
					...
					byte[] jdata = new byte[(int)entry.getSize()];
					...
					ZipInputStream innerJarInputStream
						= new ZipInputStream(new java.io.ByteArrayInputStream(jdata));
		...
		public LispObject execute(LispObject first, LispObject second) {
			...
			if (Utilities.isPlatformWindows) {
			  if (destination.isFile()) {
				...
				try {
					(new Pathname(destination.getCanonicalFile().toURI())).removeZipCacheEntry();
				} catch (IOException e) {
					e.printStackTrace();
				}
				destination.delete();
